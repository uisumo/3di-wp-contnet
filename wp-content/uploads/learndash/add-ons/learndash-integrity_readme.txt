=== LearnDash Integrity ===
Author: LearnDash
Author URI: https://learndash.com 
Plugin URI: https://learndash.com/add-on/learndash-integrity/
LD Requires at least: 3.0.0
Slug: learndash-integrity
Tags: course, protection
Requires at least: 5.0
Tested up to: 6.0
Requires PHP: 7.0
Stable tag: 1.1

Protect your course content from being shared.

== Description ==

Helps protect your course content and your website from spam, and abuse with multiple modules including:

* Hotlink protection
* Concurrent login protection
* Content protection (right click copy and paste)
* reCAPTCHA integration for v2 and v3.

= Integration Features = 

* Hotlink protection
* Concurrent login protection
* Content protection (right click copy and paste)
* reCAPTCHA integration for v2 and v3
* Hassle free setup

See the [Add-on](https://learndash.com/add-on/learndash-integrity/) page for more information.

== Installation ==

If the auto-update is not working, verify that you have a valid LearnDash LMS license via LEARNDASH LMS > SETTINGS > LMS LICENSE. 

Alternatively, you always have the option to update manually. Please note, a full backup of your site is always recommended prior to updating. 

1. Deactivate and delete your current version of the add-on.
1. Download the latest version of the add-on from our [support site](https://support.learndash.com/article-categories/free/).
1. Upload the zipped file via PLUGINS > ADD NEW, or to wp-content/plugins.
1. Activate the add-on plugin via the PLUGINS menu.

== Changelog ==

= 1.1.0 =

* Added remove hotlinking protection from .htaccess file on plugin deactivation
* Added user role selector for concurent login protection to be able to exclude user roles from protection
* Updated remove login transients when clearing session from user profile
* Fixed display of captcha box
* Fixed captcha setup instructions for clearer user experience

= 1.0.0 =

* Added new LearnDash integrity add-on (initial release)

View the full changelog [here](https://www.learndash.com/add-on/learndash-integrity/).