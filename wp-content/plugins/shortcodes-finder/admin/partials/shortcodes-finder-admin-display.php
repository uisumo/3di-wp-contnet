...
function sf_admin_page_handler() {
    ...
    if (isset($_GET['subpage']) && (
        $_GET['subpage'] == 'find_content' || $_GET['subpage'] == 'test_shortcode' || $_GET['subpage'] == 'find_unused' ||
...
function sf_admin_page_find_shortcode_handler() {
	...
	<form class="find_shortcode_form" method="get">
		...
		<p>
			...
			<label for="include_not_published" style="margin: 0 15px">
				<input type="checkbox" name="include_not_published" id="include_not_published" <?= (isset($_GET['include_not_published']) && ($_GET['include_not_published'] == 'on')) ? 'checked' : '' ?>>
    ...
    if (isset($_GET['shortcode_to_search'])) {
        $shortcode_to_search = ($_GET['shortcode_to_search'] != -1) ? $_GET['shortcode_to_search'] : '';
		$include_not_published = (isset($_GET['include_not_published']) && ($_GET['include_not_published'] == 'on'));

        sf_get_shortcodes_in_content('page', __('Post'), $shortcodes, $shortcode_to_search, $include_not_published);	// $shortcodes array passed by reference
        sf_get_shortcodes_in_content('post', __('Page'), $shortcodes, $shortcode_to_search, $include_not_published);
  						...
  						<?php foreach ($shortcode_uses as $shortcode_use) : ?>
  							<div class="shortcode_use shortcode_use_status_<?= $shortcode_use['post']['status'] ?>">
								<span class="shortcode_use_buttons float_right">
  								...
  								<b><?= $shortcode_use['post']['type'] ?>: </b><a href="<?= get_permalink($shortcode_use['post']['ID']) ?>"><?= $shortcode_use['post']['title'] ?></a>
								<a href="<?= get_edit_post_link($shortcode_use['post']['ID']) ?>">
...
function sf_admin_page_find_content_handler() { ?>
	...
	<form class="find_shortcode_form" method="post">
		<input type="hidden" name="page" value="<?= SHORTCODES_FINDER_PLUGIN_SLUG ?>">
		<input type="hidden" name="subpage" value="<?= $_GET['subpage'] ?>">
		...
		<p>
			...
			<label for="include_not_published" style="margin: 0 15px">
				<input type="checkbox" name="include_not_published" id="include_not_published" <?= (isset($_POST['include_not_published']) && ($_POST['include_not_published'] == 'on')) ? 'checked' : '' ?>>
				<?= __('Include not published contents', 'shortcodes-finder') ?>
				<span class="shortcodes-finder-tooltip tooltip-help">
					<span class="tooltiptext minwidth"><?= __('The plugin will search in drafts, future, pending and private contents.', 'shortcodes-finder') ?></span>
				</span>
				...
				<div class="shortcode_accordion_panel">
					...
					<?php foreach ($shortcode_uses as $shortcode_use) : ?>
						<div class="shortcode_use shortcode_use_status_<?= $shortcode_use['post']['status'] ?>">
							...
							<a href="<?= get_permalink($shortcode_use['post']['ID']) ?>"><?= $shortcode_use['post']['title'] ?></a>
							<a href="<?= get_edit_post_link($shortcode_use['post']['ID']) ?>">
								<span title="<?= __('Edit content', 'shortcodes-finder') ?>" class="dashicons dashicons-edit-page" aria-hidden="true"></span>
							</a>
						</div>
	...
	if (is_array($shortcodes))
		foreach ($shortcodes as $shortcode_name => $shortcode_uses) {
			...
			foreach ($shortcode_uses as $shortcode_use) {
				$shortcode_use['edit_post_link'] = get_edit_post_link($shortcode_use['post']['ID']);
				$shortcode_use['post_link'] = get_permalink($shortcode_use['post']['ID']);
...
function sf_admin_page_test_shortcode_handler() {
    ...
    if (isset($_GET['shortcode_test_shortcode'])) {
        $shortcode = $_GET['shortcode_test_shortcode'];
        $parameters = (strlen($_GET['shortcode_test_parameters']) > 0) ? stripslashes(urldecode($_GET['shortcode_test_parameters'])) : '';
        $content = (strlen($_GET['shortcode_test_content']) > 0) ? stripslashes(urldecode($_GET['shortcode_test_content'])) : '';
    } else {
        $parameters = '';
        $content = '';
    } ?>
		...
		<table class="form-table">
			<tbody>
				...
				<tr>
					...
					<td><textarea rows="5" id="shortcode_test_parameters" name="shortcode_test_parameters"><?= htmlentities($parameters) ?></textarea></td>
				</tr>
				<tr>
					<th scope="row"><label for="shortcode_test_content"><?= __('Insert content (optional)', 'shortcodes-finder') ?></label></th>
					<td><textarea rows="5" id="shortcode_test_content" name="shortcode_test_content"><?= htmlentities($content) ?></textarea></td>
				</tr>
				<tr>
					<th scope="row"></th>
...
function sf_admin_page_settings_handler() {
    ...
    if (isset($_POST['save'])) {
        ...
        $sf_settings_disable_existing_shortcodes =
          isset($_POST['sf_settings_disable_existing_shortcodes']) && ($_POST['sf_settings_disable_existing_shortcodes'] == 'on') ? true : false;
        $sf_settings_disable_unused =
          isset($_POST['sf_settings_disable_unused']) && ($_POST['sf_settings_disable_unused'] == 'on') ? true : false;
        ...
        if (wp_verify_nonce($_POST['nonce'], 'shortcodes-finder-settings-save')) {
            update_option(
		...
		<div class="scribit_plugins">
			...
			<div class="plugin-card plugin-card-<?php echo $plugin_slug ?>">
				<div class="plugin-card-top">
					<div class="name column-name">
						<h3>
							<a href="<?php echo admin_url( 'plugin-install.php?tab=plugin-information&plugin='. $plugin_slug) ?>">Proofreading<img src="https://ps.w.org/<?php echo $plugin_slug ?>/assets/icon-256x256.png" class="plugin-icon"></a>
						</h3>
					</div>
					<div class="action-links">
						<ul class="plugin-action-buttons">
							...
							<?php else: ?>
								<li><a href="<?php echo admin_url( 'plugin-install.php?s='. $plugin_slug .'+scribit&tab=search&type=term' )?>" class="button button-primary"><?php echo esc_html__( 'Install') ?></a></li>
							<?php endif; ?>
							<li><a href="<?php echo admin_url( 'plugin-install.php?tab=plugin-information&amp;plugin='. $plugin_slug) ?>" class="thickbox open-plugin-details-modal"><?php echo esc_html__('More Details') ?></a></li>
						</ul>
					</div>
			...
			<div class="plugin-card plugin-card-<?php echo $plugin_slug ?>">
				<div class="plugin-card-top">
					<div class="name column-name">
						<h3>
							<a href="<?php echo admin_url( 'plugin-install.php?tab=plugin-information&plugin='. $plugin_slug) ?>">Random<img src="https://ps.w.org/<?php echo $plugin_slug ?>/assets/icon-256x256.png" class="plugin-icon"></a>
						</h3>
					</div>
					<div class="action-links">
						<ul class="plugin-action-buttons">
...
function sf_admin_page_shortcodes_documentation_handler() {
	...
	// Form options
	$object_to_search = (isset($_GET['object_to_search']) && $_GET['object_to_search'] != -1) ? $_GET['object_to_search'] : '';
	$load_attributes = (isset($_GET['load_attributes']) && $_GET['load_attributes'] == 'on');
