	...
	$(function() {
		...
		function processRoutine(){
			...
			switch (ajax_vars.action) {
				case 'find_content': action = 'shortcodes_finder_content_search_process'; break;
				case 'find_unused': action = 'shortcodes_finder_unused_search_process'; break;
			}
								...
								'<div class="shortcode_accordion_panel">';
									...
									for (i = 0; i < shortcode_uses.length; i++) {
										...
										accordions_html += '<div class="shortcode_use shortcode_use_status_'+ shortcode_use['post']['status'] +'">'+
												'<span class="shortcode_use_buttons">' +
													'<a href="' + shortcode_use['edit_post_link'] + '">' + img_edit_post + '</a>' +
												'</span>' +
												'<p class="shortcode_code">' + shortcode_use['code'] + '</p>' +
												'<a href="' + shortcode_use['post_link'] + '">' + shortcode_use['post']['title'] + '</a>' +
											'</div>';
									}
			...
			$.post(
				...
				function( response ) {
					...
					if( response === '-1' ) {
						...
						switch(ajax_vars.action){
							case 'find_content': manage_result_find_content(response); break;
							case 'find_unused': manage_result_find_unused(response); break;
							default: manage_result_default(response); break;
						}
					}
		...
		function set_settings_existing_shortcodes_state(el){
			if(el.checked) {
				$('#sf_settings_existing_shortcodes').stop(true, false).animate({
					height: $("#sf_settings_existing_shortcodes").get(0).scrollHeight	// Set to auto
				}, 'fast');
			}
			else {
				$('#sf_settings_existing_shortcodes').stop(true, false).animate({
