...
class GF_Field_Discount extends GF_Field_Subtotal {
	...
	public static function add_discounts( $order, $form, $entry ) {
		...
		foreach ( $discount_fields as $discount_field ) {
			...
			if ( $discount_field->discountAmountType === 'flat' && $discount_field->discountProductsType === 'include' && ! empty( $discount_field->discountProducts ) ) {
				$discount = min( gp_ecommerce_fields()->get_products_total( $order, $discount_field->discountProducts, false, $form, $entry ), $discount );
			}
