		...
		if ( empty($_REQUEST["post_id"]) || empty($_REQUEST["enable"]) || !isset($_REQUEST['gb_nonce']) || !wp_verify_nonce( $_REQUEST['gb_nonce'], plugin_basename( __FILE__ ) ) || !current_user_can( 'edit_post', $_REQUEST["post_id"] ) )
			...
			if(empty($params["src"]))
				...
				if(empty($code_path))
					...
					if(strpos($file_path, "story.html") !== false && strpos($file_path, "story_html5.html") === false)
						$additional_file_paths[] = str_replace("story.html", "story_html5.html", $file_path);	//Storyline old
					else
					if(strpos($file_path, "index_lms.html") !== false && strpos($file_path, "index_lms_html5.html") === false)
						$additional_file_paths[] = str_replace("index_lms.html", "index_lms_html5.html", $file_path);	//Storyline new
					...
					if($params["content_tool"] == "articulate_rise" && strpos($file_path, "indexAPI.html") !== false && file_exists(dirname(dirname($file_path))."/scormcontent/index.html"))
						$additional_file_paths[] = dirname(dirname($file_path))."/scormcontent/index.html";	//Rise SCORM

					if($enable) {
						$response_text = __("Enable Copy Protect", "grassblade");
						foreach ($additional_file_paths as $key => $additional_file_path) {
							$response = $this->gb_content_add_code($additional_file_path, $code_path);
						}
						$response = $this->gb_content_add_code($file_path, $code_path);
					}
					else {
						$response_text = __("Disable Copy Protect", "grassblade");
						foreach ($additional_file_paths as $key => $additional_file_path) {
							$response = $this->gb_content_remove_code($additional_file_path, $code_path);
						}
						$response = $this->gb_content_remove_code($file_path, $code_path);
					}
	...
	*/
	...
	static function get_post_xapi_contents($post_ids, $with_completion_tracking_enabled_only = false) {
		...
		if(is_numeric($post_ids)) {
			...
			$all_content_ids 	= $wpdb->get_col($wpdb->prepare("SELECT meta_value FROM $wpdb->postmeta WHERE meta_key IN ('".implode("','", $keys)."') AND post_id = '%d' AND meta_value > 0", $post_id));
		}
		else if(is_array($post_ids)) {
			$post_ids = array_map("intval", $post_ids);
			$all_content_ids 	= $wpdb->get_col("SELECT meta_value FROM $wpdb->postmeta WHERE meta_key IN ('".implode("','", $keys)."') AND post_id IN (".implode(",", $post_ids).") AND meta_value > 0");
		}
