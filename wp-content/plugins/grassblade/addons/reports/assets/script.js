...
function grassblade_report_show_content_list(response) {
	...
	jQuery.each(response, function(i, u) {
		...
		html += "<div><input type='checkbox' name='content[]' value='" + u.ID + "' CHECKED /> " + u.name + "</div>";
		else
		html += "<div><input type='checkbox' name='content[]' value='" + u.ID + "' /> " + u.name + "</div>";
	});
	if(html == "")
...
//	console.log(search_text);
	var zero_option = '';
	var count = 0;
	jQuery("#nss_report_contents > div").show();
	jQuery("#nss_report_contents > div").each(function(i, v) {
...
function grassblade_report_selected() {
	...
	if(typeof GB_REPORTS.defaults.group_id != "undefined") {
