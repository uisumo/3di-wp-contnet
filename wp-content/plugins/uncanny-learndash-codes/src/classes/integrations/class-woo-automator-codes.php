...
class Automator_Codes extends Config {
	...
	public function __construct() {
		...
		if ( SharedFunctionality::is_active( 'woocommerce' ) ) {
			...
			add_filter( 'woocommerce_quantity_input_max', array(
				$this,
			...
			add_action( 'woocommerce_process_product_meta_' . $this->product_type, array(
				$this,
				'save_option_field',
			), 999 );
	...
	public function wc_qty_add_to_cart_validation( $passed, $product_id, $quantity ) {
		...
		if ( $quantity > $product_max ) {
			...
			$passed = false;
			// Display a message.
			wc_add_notice( sprintf( esc_html__( "You can't have more than %d items in cart of %s", "uncanny-learndash-codes" ), $product_max, get_the_title( $product_id ) ), "error" );
		}
	...
	public function show_out_of_stock() {
		...
		if ( $this->product_type === $product->get_type() ) {
			if ( SharedFunctionality::get_available_codes_by_group_id( $product->get_id() ) <= 0 ) {
				...
				<?php do_action( 'woocommerce_after_stock_status' );
			}
		}
	}
	...
	public function generate_csv( $order_id ) {
		...
		$file_name = apply_filters( 'uncanny-codes-filter-csv-filename', $file_name, $order );
...
WHERE c.order_id={$order_id}";
	...
	public function codes_table_after_order_table( WC_Order $order ) {
		...
		$statuses = apply_filters( 'ulc_woocommerce_allowed_order_statuses_for_codes', array(
			'processing',
			'completed',
		) );
		...
		<div style="margin-bottom: 40px;">
			<table class="td" cellspacing="0" cellpadding="6"
				...
				$total = $wpdb->get_var( "SELECT COUNT(c.code)
FROM {$table} c
    LEFT JOIN $table2 g
        ON c.code_group = g.ID
WHERE c.order_id={$order_id}" );
			...
			if ( $product->is_type( $this->product_type ) ) {
				...
				if ( absint( $codes_group ) ) {
					...
					if ( ! empty( $batch ) ) {
						...
						$order->add_order_note( sprintf( esc_html__( '%d added to order', 'uncanny-learndash-codes' ), $code ) );
					} else {
						$order->add_order_note( sprintf( esc_html__( 'Code group (#%d) associated with %s (#%d) does not exist anymore.', 'uncanny-learndash-codes' ), $codes_group, $product_name, $product_id ) );
					}
				} else {
					$order->add_order_note( sprintf( esc_html__( 'Code group not set for %s (#%d)', 'uncanny-learndash-codes' ), $product_name, $product_id ) );
				}
			}
		}
	}
