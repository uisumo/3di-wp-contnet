...
class AdminMenu extends Boot {
	...
	public static function register_options_menu_page() {
		...
		add_submenu_page( $menu_slug, esc_html__( 'View codes', 'uncanny-learndash-codes' ), esc_html__( 'View codes', 'uncanny-learndash-codes' ), $capability, 'uncanny-learndash-codes', array(
			__CLASS__,
			'options_menu_view_codes',
		) );

		add_submenu_page( $menu_slug, $sub_menu_title, $sub_menu_title, $capability, 'uncanny-learndash-codes-create', array(
			__CLASS__,
			'options_menu_page_output',
		) );
		add_submenu_page( $menu_slug, esc_html__( 'Settings', 'uncanny-learndash-codes' ), esc_html__( 'Settings', 'uncanny-learndash-codes' ), $capability, 'uncanny-learndash-codes-settings', array(
			__CLASS__,
				...
				<div class="ulc__admin-content">
					...
					<?php } elseif ( SharedFunctionality::ulc_filter_has_var( 'force_downloaded' ) ) { ?>
						...
						<div class="notice error">
							...
							<a target="_blank"
							   href="<?php echo add_query_arg( array( 'mode' => 'download_file' ), remove_query_arg( array(
								   'redirect_nonce',
								   'mode',
								   'saved',
								   'force_downloaded',
					...
					<form method="post" action="" id="uncanny-learndash-codes-form">
						...
						if ( is_multisite() ) {
							...
							$term_conditions          = get_blog_option( get_current_blog_id(), Config::$uncanny_codes_settings_term_conditions, null );
							$autocomplete_settings    = get_blog_option( get_current_blog_id(), Config::$uncanny_codes_settings_autocomplete, null );
						} else {
							...
							$term_conditions          = get_option( Config::$uncanny_codes_settings_term_conditions, null );
							$autocomplete_settings    = get_option( Config::$uncanny_codes_settings_autocomplete, null );
						}
						// Gravity Forms.
