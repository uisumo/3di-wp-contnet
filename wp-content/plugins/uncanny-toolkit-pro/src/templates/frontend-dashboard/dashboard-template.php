                                                    ...
                                                    <a class="<?php echo wp_kses( $lesson['status'], $allowed_html ); ?>"
                                                       href="<?php echo esc_attr( learndash_get_step_permalink( $lesson['post']->ID, $course_id ) ); ?>"><?php echo wp_kses( $lesson['post']->post_title, $allowed_html ); ?>
														...
														if ( ! empty( $lesson['lesson_access_from'] ) ) {
															$lesson_available_from  = ld_lesson_access_from( $lesson['post']->ID, $user_id );
															...
															if ( ! empty( $uncanny_active_classes ) ) {
																if ( key_exists( 'uncanny_pro_toolkit\UncannyDripLessonsByGroup', $uncanny_active_classes ) ) {
																	...
																	$lesson_access_from = uncanny_pro_toolkit\UncannyDripLessonsByGroup::get_lesson_access_from( $uo_lesson_id, $user_id );
															...
															?>
                                                            <small class="notavailable_message"> <?php echo wp_kses( sprintf( esc_attr__( 'Available on: %s', 'uncanny-pro-toolkit' ), date_i18n( get_option( 'date_format' ), $lesson_available_from ) ), $allowed_html ); ?> </small>
														<?php }
														?></a>
													<?php
													//													/* Lesson Topics */
							...
							if ( ! empty( $quiz_attempts[ $course->ID ] ) ) { ?>
                                <div class="learndash_profile_quizzes clear_both">
                                    <div class="learndash_profile_quiz_heading">
                                        ...
                                        <div class="statistics"><?php esc_attr_e( 'Statistics', 'uncanny-pro-toolkit' ); ?></div>
                                        <div
...
//                                        }
										...
										if ( ! empty( $quiz_title ) ) {
                                            ...
                                            <div class="<?php echo wp_kses( $status, $allowed_html ); ?>">
                                                ...
                                                <div class="statistics">
													<?php
													if ( ( ( (int) $user_id === (int) get_current_user_id() ) || ( learndash_is_admin_user() ) || ( learndash_is_group_leader_user() ) ) && ( isset( $quiz_attempt['statistic_ref_id'] ) ) && ( ! empty( $quiz_attempt['statistic_ref_id'] ) ) ) {
														/**
														 * @since 2.3
                                                        ...
                                                        <a class="user_statistic"
                                                           data-statistic_nonce="<?php echo wp_create_nonce( 'statistic_nonce_' . $quiz_attempt['statistic_ref_id'] . '_' . $user_id . '_' . $user_id ); ?>"
			...
			if ( isset( $atts['no_courses_message'] )  && ! empty( $atts['no_courses_message'] ) ) { ?>
				<div class="uo-dashboard no-courses-message legacy"><?php echo esc_html($atts['no_courses_message']); ?></div>
			<?php } 
		}
		?>
        <!--End #course-->
