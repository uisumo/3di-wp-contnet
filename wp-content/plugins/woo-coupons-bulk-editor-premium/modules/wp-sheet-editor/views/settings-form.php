...
<div class="wpse-settings-form-wrapper">
	...
	<form class="wpse-set-settings tabs-content" data-reload-after-success="1">
		...
		foreach ($sections as $section) {
			$section_id = sanitize_html_class($section['title']);
			?>
			<div class="<?php echo esc_attr($section_id); ?> tab-content">
				<?php
					...
					<div class="field-wrapper">
						<label for="<?php echo esc_attr($field['id']); ?>">
							<?php if ($field['type'] === 'switch') { ?>
								...
								<input id="<?php echo esc_attr($field['id']); ?>"  name="settings[<?php echo esc_attr($field['id']); ?>]" type="checkbox" value="1" <?php checked(1, (int) $value); ?> />
							<?php } ?> 
						...
						<?php if ($field['type'] === 'text') { ?>
							<input id="<?php echo esc_attr($field['id']); ?>" name="settings[<?php echo esc_attr($field['id']); ?>]" value="<?php echo esc_attr($value); ?>" type="<?php echo esc_attr($input_type); ?>" />
						<?php } ?>
						<?php if ($field['type'] === 'textarea') { ?>
							<textarea id="<?php echo esc_attr($field['id']); ?>" name="settings[<?php echo esc_attr($field['id']); ?>]"><?php echo esc_attr($value); ?></textarea>
						<?php } ?>		
						<?php
