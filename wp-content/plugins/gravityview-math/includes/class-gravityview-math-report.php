...
class GravityView_Math_Report {
	...
	public function add_gf_logging( $plugins = array() ) {
		...
		$plugins['gravityview_math'] = esc_html__( 'Math by GravityView', 'gravityview-math' );
	...
	private function get_debug( $atts ) {
		 ...
		 * @param GravityView_Math_Report $this The report.
		 */
	...
	public function log_from_action( $message = '', $data = [] ) {
		...
		if ( 'errors' === $type ) {
			$klogger_level = KLogger::ERROR;
		} else {
			$klogger_level = KLogger::DEBUG;
		}

		GFLogging::log_message( 'gravityview_math', $message, $klogger_level );
	}
	...
	private function generate_message( $message, $data = [], $type = '' ) {
		...
		$default_value_info = isset( $atts['default_value'] ) && '' !== $atts['default_value'] && 'skip' !== $atts['default_value'] ? __( 'The supplied default value was used', 'gravityview-math' ) . ': ' . $atts['default_value'] : '';
		...
		switch ( $code ) {
			...
			case 'ID_not_set':
				$final_message   = sprintf( '%s, %s', $message, sprintf( __( '%s scope requires an ID to process the formula.', 'gravityview-math' ), '<code>' . $atts['scope'] . '</code>' ) );
				self::$skip_flag = true;
				break;
	 ...
	 * @return void
	 */
	public function add_alert( $type = 'warnings', $alert, $line_number = 0 ) {
