	...
	for ( $i = 0; $i < $count; $i++ ) {
      ...
      /* Saves the member if at least one of these fields are not empty. */
        ...
        (isset($member_scl_type4) && $member_scl_type4) ? $new_team[$i]['_tmm_sc_type4'] = stripslashes( strip_tags( sanitize_text_field( $member_scl_type4 ) ) ) : $new_team[$i]['_tmm_sc_type4'] = '';
				...
				(isset($member_color) && $member_color) ? $new_team[$i]['_tmm_color'] = stripslashes( strip_tags( sanitize_text_field( $member_color ) ) ) : $new_team[$i]['_tmm_color'] = '';

				(isset($member_hide) && $member_hide == '1') ? $new_team[$i]['_tmm_hide'] = stripslashes( strip_tags( sanitize_text_field( $member_hide ) ) ) : $new_team[$i]['_tmm_hide'] = '0';
