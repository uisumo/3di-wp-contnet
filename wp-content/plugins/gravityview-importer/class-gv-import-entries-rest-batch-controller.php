	...
	public function can_create_batch( $request ) {
		...
		if ( ! \GFCommon::current_user_can_any( $required_cap ) ) {
			return new \WP_Error(
				...
				__( "Sorry, you don't have adequate permissions to import entries.", 'gravityview-importer' ),
		...
		if ( ! $request->get_param( 'form_id' ) || $request->get_param( 'form_title' ) ) {
			...
			if ( ! \GFCommon::current_user_can_any( 'gravityforms_create_form' ) ) {
				return new \WP_Error(
					...
					__( "Sorry, you don't have adequate permissions to create a new form.", 'gravityview-importer' ),
					array( 'status' => rest_authorization_required_code() )
				);
			}
		}
