...
class SharedFunctions {
	...
	/**
	 * We are run ctype_alnum() as a check on codes. Do not add special characters.
	 * @var string
	 */
	...
	public static function add_group_seats_link( $group_id, $amount_seats ) {
		...
		return "$site?modify-group=true&modify-group-id=$group_id&new-qty=$amount_seats";
	}
		...
		if ( is_array( $code_details ) ) {
			if ( 'failed' === $code_details['result'] ) {
				if ( 'invalid' === $code_details['error'] ) {
					return array( 'is_valid' => false, 'message' => Config::$invalid_code );

				} elseif ( 'existing' === $code_details['error'] ) {
					return array( 'is_valid' => false, 'message' => Config::$already_redeemed );
				}
