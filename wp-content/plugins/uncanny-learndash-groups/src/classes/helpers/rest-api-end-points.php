...
class RestApiEndPoints {
	...
	public function __construct() {
		...
		add_action( 'wp_ajax_get_group_management_users', array( __CLASS__, 'datatable_qrys' ), 99 );
	}
	...
	public static function datatable_qrys( $all = false, $group_id_ = false ) {
		...
		if ( ulgm_filter_has_var( 'order' ) ) {
			...
			$first_order = array_shift( ulgm_filter_input( 'order' ) );
			if ( $first_order ) {
	...
	public static function register_routes() {
		...
		register_rest_route( ULGM_REST_API_PATH, '/add_user/', array(
			'methods'             => 'POST',
			'callback'            => [ __CLASS__, 'add_user' ],
			'permission_callback' => function () {
				return self::permission_callback_check();
			},
		) );

		register_rest_route( ULGM_REST_API_PATH, '/add_group_leader/', array(
			'methods'             => 'POST',
			'callback'            => [ __CLASS__, 'add_group_leader' ],
			'permission_callback' => function () {
				return self::permission_callback_check();
			},
		) );

		register_rest_route( ULGM_REST_API_PATH, '/remove_group_leaders/', array(
			'methods'             => 'POST',
			'callback'            => [ __CLASS__, 'remove_group_leaders' ],
			'permission_callback' => function () {
				return self::permission_callback_check();
			},
		) );

		register_rest_route( ULGM_REST_API_PATH, '/upload_users/', array(
			'methods'             => 'POST',
			'callback'            => [ __CLASS__, 'upload_users' ],
			'permission_callback' => function () {
				return self::permission_callback_check();
			},
		) );

		register_rest_route( ULGM_REST_API_PATH, '/email_users/', array(
			'methods'             => 'POST',
			'callback'            => [ __CLASS__, 'email_users' ],
			'permission_callback' => function () {
				return self::permission_callback_check();
			},
		) );

		register_rest_route( ULGM_REST_API_PATH, '/remove_users/', array(
			'methods'             => 'POST',
			'callback'            => [ __CLASS__, 'remove_users' ],
			'permission_callback' => function () {
				return self::permission_callback_check();
			},
		) );

		register_rest_route( ULGM_REST_API_PATH, '/download_keys_csv/', array(
			'methods'             => 'POST',
			'callback'            => [ __CLASS__, 'download_keys_csv' ],
			'permission_callback' => function () {
				return self::permission_callback_check();
			},
		) );

		register_rest_route( ULGM_REST_API_PATH, '/send-password-reset/', array(
			'methods'             => 'POST',
			'callback'            => [ __CLASS__, 'send_password_reset' ],
			'permission_callback' => function () {
				return self::permission_callback_check();
			},
		) );

		register_rest_route( ULGM_REST_API_PATH, '/get_user_details/', [
			'methods'             => 'POST',
			'callback'            => [ __CLASS__, 'get_user_details' ],
			'permission_callback' => function () {
				return self::permission_callback_check();
			},
		] );

		register_rest_route( ULGM_REST_API_PATH, '/edit_user/', [
			'methods'             => 'POST',
			'callback'            => [ __CLASS__, 'update_user_details' ],
			'permission_callback' => function () {
				return self::permission_callback_check();
			},
		] );
	...
	public static function permission_callback_check( $admin_only = false ) {
		...
		$allowed_roles = apply_filters( 'ulgm_rest_api_callback_roles', array( 'administrator', 'group_leader' ) );
	...
	public static function email_users( WP_REST_Request $request ) {
		if ( ( $request->has_param( 'group_email_nonce' ) ) && ( $request->has_param( 'group_email_group_id' ) ) && ( ! empty( $request->get_param( 'group_email_group_id' ) ) ) ) {
			...
			$group_email_data['statuses'] = $request->has_param( 'group_email_status' ) ? $request->get_param( 'group_email_status' ) : array();
			...
			$group_email_data['group_id'] = intval( $request->get_param( 'group_email_group_id' ) );
			...
			if ( ( ! $request->has_param( 'group_email_nonce' ) ) || ( empty( $request->get_param( 'group_email_nonce' ) ) )
			...
			if ( ! empty( $group_email_data['statuses'] ) ) {
				...
				if ( in_array( 'not-enrolled', $group_email_data['statuses'] ) ) {
					global $wpdb;
					$codes_group_id = $wpdb->get_var(
						$wpdb->prepare(
							'SELECT ID FROM ' . $wpdb->prefix . SharedFunctions::$db_group_tbl . ' WHERE  ld_group_id = %d', $group_email_data['group_id']
						)
					);
					...
					if ( absint( $codes_group_id ) ) {
						...
						$temp_users_code = $wpdb->get_results( $wpdb->prepare( $q, $codes_group_id, SharedFunctions::$not_redeemed_status ) );
						foreach ( $temp_users_code as $user ) {
							...
							$not_enrolled_users[ $email ] = (object) [
								'first_name' => $f,
								'last_name'  => $l,
								'email'      => $email,
							];
						...
						if ( ! empty( $mail_args ) ) {
							...
							foreach ( $email_addresses as $k => $email_address ) {
								...
								if ( ! $ulgm_gdpr_compliance ) {
									...
									if ( $send_mail ) {
										$mail_ret        = SharedFunctions::wp_mail( $email_address,
											$mail_args_subject,
											$mail_args_message,
											Group_Management_Helpers::get_headers( true, $group_leader_details ) );
	...
	public static function remove_users( WP_REST_Request $request ) {
		...
		if ( $request->has_param( 'group-id' ) ) {
			...
			if ( ! in_array( $request->get_param( 'group-id' ), $user_group_ids ) ) {
...
//					}
					//}
					if ( $status === SharedFunctions::$not_started_status || 'yes' === get_option( 'allow_to_remove_users_anytime', 'no' ) ) {
	...
	public static function remove_group_leaders( WP_REST_Request $request ) {
		...
		if ( $request->has_param( 'group-id' ) ) {
			...
			$user_group_ids = LearndashFunctionOverrides::learndash_get_administrators_group_ids( get_current_user_id(), true );
			...
			if ( 'remove-group-leaders' === $action ) {
				...
				if ( $request->has_param( 'removing-group-leaders' ) ) {
					$group_leaders_new = array();
					$users             = explode( ',', $request->get_param( 'removing-group-leaders' ) );

					$current_group_leaders_ids = LearndashFunctionOverrides::learndash_get_groups_administrator_ids( $group_id, true );
...
//					}
	...
	/**
	 * Save email templates for user Send enrollment key, Add and invite email and Add group leader/Create group email
	 * @since 1.0.0
	 *
	 */
	public static function download_keys_csv( WP_REST_Request $request ) {
		...
		// Does the current user have permission
		$allowed_roles = apply_filters( 'ulgm_gm_allowed_roles', array(
			'administrator',
			'group_leader',
		...
		// is the current user able to administer this group
		if ( ! in_array( $request->get_param( 'group-id' ), $user_group_ids ) && ! current_user_can( 'manage_options' ) ) {
			$data['message'] = __( 'You do not have permission to manage this group.', 'uncanny-learndash-groups' );
			$data['error']   = 'invalid-group-id';
			wp_send_json_error( $data );
		}
	...
	public static function send_password_reset( WP_REST_Request $request ) {
		...
		$users_ids_raw = explode( ',', str_replace( [
			'[',
			']',
	...
	public static function get_user_details( WP_REST_Request $request ) {
		...
		$response_data = [
			'edit-user-id'    => [ 'value' => '', 'is_editable' => true ],
			'edit-first-name' => [ 'value' => '', 'is_editable' => true ],
			'edit-last-name'  => [ 'value' => '', 'is_editable' => true ],
			'edit-email'      => [ 'value' => '', 'is_editable' => true ],
			'edit-username'   => [ 'value' => '', 'is_editable' => true ],
		];
	...
	public static function validate_new_user_data( $action, WP_REST_Request $request ) {
		...
		if ( $request->has_param( 'group-id' ) ) {
			...
			// is the current user able to administer this group
			if ( ! in_array( $request->get_param( 'group-id' ), $user_group_ids, true ) && ! current_user_can( 'manage_options' ) ) {
				$data['message'] = __( 'You do not have permission to manage this group.', 'uncanny-learndash-groups' );
				$data['error']   = 'invalid-group-id';
				wp_send_json_error( $data );
			}
