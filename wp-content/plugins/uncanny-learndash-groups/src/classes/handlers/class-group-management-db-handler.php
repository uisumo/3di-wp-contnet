...
class Group_Management_DB_Handler {
	...
	public function add_code_group( $attr ) {
		...
		$wpdb->insert( $wpdb->prefix . ulgm()->db->tbl_group_details,
			$insert,
			array(
				'%d',
				'%d',
	...
	public function ld_group_id_from_order_id( $order_id = null ) {
		...
		return $wpdb->get_var(
			$wpdb->prepare(
				'SELECT ld_group_id FROM  ' . $wpdb->prefix . ulgm()->db->tbl_group_details . ' WHERE order_id=%d', $order_id
			)
		);
	}
	...
	public function set_sign_up_code_status( $data = array() ) {
		...
		$codes_group_id = isset( $data['codes_group_id'] ) ? $data['codes_group_id'] : '';
		...
		if ( null !== $user_id ) {
			$update = $wpdb->update(
				$wpdb->prefix . ulgm()->db->tbl_group_codes,
				array( 'student_id' => $user_id, 'used_date' => current_time( 'mysql' ) ),
				array( 'code' => $code ),
				array( '%d', '%s' ),
				array( '%s' )
			);
...
ON c.group_id = g.ID AND g.ld_group_id = %d
WHERE c.student_id = %d $limit", $group_id, $user_id
			)
		);
	}
	...
	public function get_sign_up_code_from_group_id( $group_id = null, $limit = 1, $user_id = 0, $order_id = 0, $is_cron = false ) {
		...
		$sql = $wpdb->prepare(
			"SELECT `code`
					...
					AND group_id = %d
					ORDER BY RAND()
					LIMIT $limit", SharedFunctions::$available_status, $codes_group_id
		);
	...
	public function get_codes_for_download_csv( $group_id ) {
		...
		return $wpdb->get_results( $wpdb->prepare( "SELECT p.post_title AS group_name, gc.code
FROM $wpdb->prefix" . ulgm()->db->tbl_group_codes . " gc
JOIN $wpdb->prefix" . ulgm()->db->tbl_group_details . " gd
ON gd.ID = gc.group_id AND gd.ld_group_id = %d
LEFT JOIN $wpdb->posts p
ON gd.ld_group_id = p.ID
WHERE student_id IS NULL AND user_email IS NULL", $group_id ), ARRAY_A );
	}
