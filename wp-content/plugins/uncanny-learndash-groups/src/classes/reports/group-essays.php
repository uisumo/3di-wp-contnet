...
class GroupEssays {
	...
	public function create_essay_table( $request = array() ) {
		...
		<div class="uo-groups uo-reports">
			<?php if ( ! empty( SharedFunctions::get_group_management_page_id() ) && ! empty( SharedFunctions::get_group_essay_report_page_id() ) ):
				?>
								...
								<div class="uo-row uo-groups-select-filter">
									<div class="uo-select">
										...
										<select class="change-group-management-form" style=""
											...
											<option value="graded" <?php if ( 'graded' === $status ) { ?> selected <?php } ?>><?php _e( 'Graded', 'uncanny-learndash-groups' ); ?></option>
											<option value="ungraded" <?php if ( 'ungraded' === $status ) { ?> selected <?php } ?>><?php _e( 'Ungraded', 'uncanny-learndash-groups' ); ?></option>
										</select>
									</div>
								</div>
							</div>
	...
	public function essays_scripts( $request = array() ) {
		...
		if ( Utilities::has_shortcode( $post, 'uo_groups_essays' ) || Utilities::has_block( $post, 'uncanny-learndash-groups/uo-groups-essays-report' ) ) {
			...
			wp_register_script( 'ulgm-frontend', Utilities::get_asset( 'frontend', 'bundle.min.js' ), [ 'jquery', 'ulgm-datatables' ], Utilities::get_version(), true );
	...
	private function get_frontend_localized_strings() {
		...
		$localized_strings['question_text'] = __( 'Question', 'uncanny-learndash-groups' );
	...
	public function essays_table( $lesson_ID = 0, $course_ID = 0, $group_ID = 0, $quiz_ID = 0, $status = 'ungraded' ) {
		...
		if ( $status === 'all' ) {
			$q_vars['post_status'] = [ 'graded', 'not_graded' ];
		} elseif ( $status === 'ungraded' ) {
			$q_vars['post_status'] = [ 'not_graded' ];
		} elseif ( $status === 'graded' ) {
			$q_vars['post_status'] = [ 'graded' ];
		}
									...
									<div class="uo-row">
										...
										<select name="essay_quiz_post_id">
											...
											foreach ( $essay_quiz_query->posts as $quiz_post_id ) {
												?>
												<option
												value="<?php echo $quiz_post_id ?>"><?php echo get_the_title( $quiz_post_id ); ?></option><?php
											}
											?>
										</select>
									</div>
			...
			if ( ulgm_filter_has_var( 'post_status', INPUT_POST ) ) {
				if ( ( ulgm_filter_input( 'post_status', INPUT_POST ) != $submitted_essay['status'] ) ) {
					if ( ulgm_filter_input( 'post_status', INPUT_POST ) == 'graded' ) {
						$quiz_score_difference = 1;
					} else if ( ulgm_filter_input( 'post_status', INPUT_POST ) == 'not_graded' ) {
						$quiz_score_difference = - 1;
					}
				}
			}
