...
class GroupReportsInterface {
	...
	public function uo_group_course_report( $attributes, $content = '' ) {
		...
		if ( ulgm_filter_has_var( 'user-id' ) && $request['transcript-page-id'] !== 0 ) {
	...
	public function get_user_course_data() {
		...
		if ( 'get-user-data-courses' === $action ) {
			...
			// is the current user able to administer this group
			if ( ! in_array( ulgm_filter_input( 'course-id', INPUT_POST ), $group_course_ids ) ) {
			...
			$user_course_data = $this->user_course_data( $course_id, $group_id );
			...
			$group_leader_id  = get_current_user_id();
			$leader_group_ids = LearndashFunctionOverrides::learndash_get_administrators_group_ids( $group_leader_id );
			...
			foreach ( $user_group_ids as $group_id ) {
				...
				foreach ( $courses as $course ) {
					...
					$courses_collected[ $course ] = $this->user_course_data( $course, $group_id, $user_id );
	 ...
	 * @return array
	 */
	public function user_course_data( $course_id, $group_id, $user_id = false ) {
		...
		foreach ( $groups_user_object as $user ) {
			...
			//          $date_completed    = apply_filters( 'ulgm_group_course_report_user_date_completed', $date_completed, $user, $course_id );
			$percent_completed = $this->get_course_completion_percentage( $user->ID, $course_id ) . '%';
	...
	public function get_course_completion_percentage( $user_id, $course_id ) {
		if ( version_compare( LEARNDASH_VERSION, '3.4.0', '>=' ) ) {
			return $this->get_course_completion_percentage_v4( $user_id, $course_id );
		}
	 ...
	 * @return int|mixed
	 */
	public function get_course_completion_percentage_v4( $user_id, $course_id ) {
