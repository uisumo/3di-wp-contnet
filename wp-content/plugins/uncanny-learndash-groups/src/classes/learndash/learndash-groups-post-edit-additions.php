...
class LearndashGroupsPostEditAdditions {
		...
		if ( ! empty( $code_group_id ) ) {
			...
			<div class="sfwd_input " id="sfwd-group_ulgm_total_seats">
                <span class="sfwd_option_label" style="text-align:right;vertical-align:top;">
                    ...
                    <div class="sfwd_option_div">
                        <input name="_ulgm_total_seats" id="_ulgm_total_seats" type="text" size="57"
			...
			<div class="sfwd_input " id="sfwd-groups_ulgm_available_seats">
                <span class="sfwd_option_label" style="text-align:right;vertical-align:top;">
                    ...
                    <div class="sfwd_option_div">
                        <input name="_ulgm_remaining_seats" id="_ulgm_remaining_seats" disabled="disabled" type="text"
							   size="57"
							   value="<?php echo $remaining_seats; ?>" style="max-width: 100%;">
                    </div>
			...
			<?php
			$link_upgrade         = add_query_arg( array(
			...
			?>
			<div class="sfwd_input " id="sfwd-lessons_upgrade_learndash_group">
                <span class="sfwd_option_input" style="width:100%;">
                    <div class="sfwd_option_div">
                        <a id="reconcile_button" href="<?php echo $link_upgrade; ?>"
						   class="button button-primary"><?php _e( 'Reconcile seat count', 'uncanny-learndash-groups' ) ?></a>
                        <a id="reconcile_button_refresh" style="float:right; display:none" href="javascript:;"
						   onclick="window.location.reload(false);"
						   class="button button-secondary"><?php _e( 'Refresh', 'uncanny-learndash-groups' ) ?></a>
                    </div>
                    <div class="sfwd_help_text_div" style="display:none" id="uo-total-seats-count">
                        <label class="sfwd_help_text"><?php _e( 'Total number of seats available to the group administrator for dispersion.', 'uncanny-learndash-groups' ) ?></label>
                    </div>
                </span>
				<p style="clear:left"></p>
			...
			<div id="reconcile_confirmation" style="display: none;">
				<?php
				printf( __( 'This will assign a %s to every member of the group that is not currently assigned a %s and restore missing %s. Continue?', 'uncanny-learndash-groups' ), $per_seat_text, $per_seat_text, $per_seat_text_plural );
				?>
			</div>
			<script>
			...
			?>
			<div class="sfwd_input" id="sfwd-lessons_upgrade_learndash_group_add" style="<?php echo $class; ?>">
                <span class="sfwd_option_input">
                    <div class="sfwd_option_div">
                        <a href="<?php echo $link_upgrade; ?>" id="_updgrade_group"
		...
		<script>
			jQuery(document).ready(() => {
				function update_meta_box() {
					...
					if ('no' === ur) {
						<?php
						LearndashGroupsPostEditAdditions::remove_users_from_group_admin_func( $post->ID );
						?>
	...
	public function render_invite_metabox( $post ) {
		...
		<div class="sfwd_input " id="sfwd-lessons_forced_lesson_time">
			...
			<div class="sfwd_help_text_div" id="uo-invited-users">
				<label class="sfwd_help_text"><?php printf( __( 'A %s has been reserved for the users listed above, who have been sent an enrollment key but have not yet created an account.', 'uncanny-learndash-groups' ), strtolower( ulgm()->group_management->seat->get_per_seat_text() ) ) ?></label>
			</div>
			<p style="clear:left"></p>
		</div>
		<?php
	...
	public function render_metabox_for_woo( $post ) {
				...
				if ( ! empty( $order_type ) ) {
					...
					if ( $meta && isset( $meta['order_details']['order_id'] ) && is_numeric( $meta['order_details']['order_id'] ) ) {
						...
						if ( $meta && isset( $meta['order_details']['order_id'] ) && is_numeric( $meta['order_details']['order_id'] ) ) {
							...
							$order_type = sprintf( _x( 'Additional %s %s purchased.', 'Group seats count and per seat label', 'uncanny-learndash-groups' ), $new_qty, strtolower( SharedFunctions::get_per_seat_text( $new_qty ) ) );
						}
					}
				}
				if ( empty( $order_type ) ) {
		...
		if ( $other_orders ) {
			...
			<?php if ( count( $other_orders ) - $limit > 0 ) {
				$val = count( $other_orders ) - $limit; ?>
				<p>.</p>
				<p>.</p>
				<p>
					<i><?php printf( ngettext( __( '%d additional order', 'uncanny-learndash-groups' ), __( '%d other orders.', 'uncanny-learndash-groups' ), $val ), $val ) ?></i>
				</p>
				<?php
			}
		}
	...
	public static function remove_users_from_group_admin_func( $post_id, $force = false, $group_leaders = array(), $group_users = array() ) {
		...
		global $wpdb;
		$codes_users      = array();
		$sql              = $wpdb->prepare( "SELECT student_id, ID FROM {$wpdb->prefix}" . SharedFunctions::$db_group_codes_tbl . "
		                        WHERE code_status != %s
		                        AND student_id REGEXP '^-?[0-9]+$'
		                        AND group_id = %d", SharedFunctions::$available_status, $codes_group_id );
...
//
		 ...
		 * Save Seats -- Start
		 */
		$this->update_seat_count( $post_id, $code_group_id );
		/**
		 * Save Seats -- End
		 */
		 ...
		 * Remove users -- Start
		 */
		LearndashGroupsPostEditAdditions::remove_users_from_group_admin_func( $post->ID, true, $group_admins, $group_users );
		/**
		 * Remove users -- End
		 */
	}
	 ...
	 * @param $code_group_id
	 */
	public function update_seat_count( $post_id, $code_group_id ) {
		$existing_seats = ulgm()->group_management->seat->total_seats( $post_id );
		$new_seats      = absint( ulgm_filter_input( '_ulgm_total_seats', INPUT_POST ) );
		// Seats added
	...
	public function update_user_redeem_seats( $post_id ) {
		...
		$group_users  = $r['group_users'];
		$group_admins = $r['group_admins'];
		LearndashGroupsPostEditAdditions::update_user_redeemed_seat_func( $post_id, true, $group_users, $group_admins );
