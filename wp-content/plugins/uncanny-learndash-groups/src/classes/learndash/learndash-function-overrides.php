...
class LearndashFunctionOverrides {
	...
	public static function learndash_get_groups_user_ids( $group_id = 0, $bypass_transient = false ) {
		...
		$qry     = $wpdb->prepare( "SELECT user_id
										FROM $wpdb->usermeta
										WHERE meta_key LIKE %s
										AND meta_value = %d", "learndash_group_users_{$group_id}", $group_id );
	...
	public static function learndash_get_groups_administrator_ids( $group_id = 0, $bypass_transient = false ) {
		...
		$qry     = $wpdb->prepare( "SELECT user_id
										FROM $wpdb->usermeta
										WHERE meta_key LIKE %s
										AND meta_value = %d", "learndash_group_leaders_{$group_id}", $group_id );
		$results = $wpdb->get_col( $qry );
	...
	public static function learndash_get_administrators_group_ids( $user_id = 0 ) {
		...
		return $wpdb->get_col( $wpdb->prepare( "SELECT meta_value FROM $wpdb->usermeta WHERE meta_key LIKE %s AND user_id = %d", 'learndash_group_leaders_%%', $user_id ) );
	}
