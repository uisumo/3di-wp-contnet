...
class WoocommerceLicense {
	...
	public function __construct() {
		...
		add_action( 'uo_new_group_purchased', array( $this, 'uo_new_group_purchased_func' ), 10, 2 );

		add_action( 'pre_get_posts', array( $this, 'hide_license_product_from_searches' ) );
		// WP No index
		add_action( 'wp_robots', array( $this, 'manually_add_noindex_to_licenses' ), PHP_INT_MAX );
	}
	...
	public function plugins_loaded() {
		...
		add_filter( 'woocommerce_product_get_price', array( $this, 'return_custom_price' ), 9, 2 );
		...
		add_filter( 'woocommerce_cart_item_thumbnail', array( $this, 'woocommerce_cart_item_thumbnail_func' ), 99, 3 );
		add_filter( 'woocommerce_cart_item_permalink', array( $this, 'woocommerce_cart_item_permalink_func' ), 99, 3 );
		add_filter( 'woocommerce_order_item_meta_end', array( $this, 'woocommerce_order_item_meta_end_func' ), 10, 3 );
		...
		/************************************/
		add_action( 'woocommerce_checkout_process', [ $this, 'process_group_name' ], 30 );
		add_action( 'woocommerce_license_add_to_cart', [ $this, 'woocommerce_simple_add_to_cart' ], 30 );
		add_action( 'woocommerce_product_data_panels', array( $this, 'license_options_product_tab_content' ) );
		add_action( 'woocommerce_order_status_completed', [ $this, 'process_license_odr_complete' ], 99 );
		//add_action( 'woocommerce_before_calculate_totals', array( $this, 'woo_license_price_update' ), 20 );
		add_action( 'woocommerce_checkout_update_order_meta', [ $this, 'grp_update_order_meta' ] );
		add_action( 'woocommerce_process_product_meta_license', array( $this, 'save_license_option_field' ) );
		add_action( 'woocommerce_process_product_meta_variable_license', [ $this, 'save_license_option_field' ] );
		add_action( 'woocommerce_admin_order_data_after_billing_address', [ $this, 'grp_admin_ordr_billing_address' ] );
		...
		if ( 'yes' === get_option( 'add_courses_as_part_of_license', 'no' ) ) {
			...
			add_action( 'woocommerce_before_calculate_totals', array( $this, 'deduct_courses_in_cart' ), 29, 1 );
		}
	}
	...
	public function license_options_product_tab_content() {
		...
		<div class='options_group show_if_license show_if_subscription'><?php
			...
			$this->woocommerce_wp_select_multiple( array(
					...
					'description' => sprintf( __( 'Hold down Ctrl (Command on Mac) and click to select multiple LearnDash Group %s products. This will make this product a Subscription Group License product and purchases will create a new LearnDash group. If the box above is empty, first create at least one LearnDash Group %s product. See %s for details.', 'uncanny-learndash-groups' ), \LearnDash_Custom_Label::get_label( 'courses' ), \LearnDash_Custom_Label::get_label( 'course' ), $kb ),
					'options'     => $license,
			) );
			?></div>
	...
	public function process_license_odr_complete( $order_id ) {
		...
		foreach ( $line_items as $line ) {
			$courses_linked = apply_filters(
					'ulgm_license_group_courses_linked_in_order',
					get_post_meta( $line['product_id'], SharedFunctions::$license_meta_field, true ),
					$group_id,
					$this
			);
			if ( $courses_linked ) {
				foreach ( $courses_linked as $course_product ) {
					$course_id = apply_filters(
							...
							get_post_meta( $course_product, '_ulgm_course', true ),
							$group_id,
							$this
					);
	...
	public function create_group_related_fields( $checkout ) {
		...
		$group_name_args = apply_filters( 'ulgm_group_name_args', array(
				'type'         => 'text',
				'class'        => $classes,
