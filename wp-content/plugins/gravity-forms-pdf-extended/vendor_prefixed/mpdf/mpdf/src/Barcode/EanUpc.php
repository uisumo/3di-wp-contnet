 ...
 */
class EanUpc extends \GFPDF_Vendor\Mpdf\Barcode\AbstractBarcode implements \GFPDF_Vendor\Mpdf\Barcode\BarcodeInterface
{
    /**
     * @param string $code
     * @param int $length
    ...
    private function init($code, $length)
    {
        if (\preg_match('/[\\D]+/', $code)) {
            throw new \GFPDF_Vendor\Mpdf\Barcode\BarcodeException(\sprintf('Invalid EAN UPC barcode value "%s"', $code));
        }
        $upce = \false;
        $checkdigit = \false;
        if ($length == 6) {
        ...
        if ($upce) {
            ...
            if ($invalidUpce) {
                throw new \GFPDF_Vendor\Mpdf\Barcode\BarcodeException('UPC-A cannot produce a valid UPC-E barcode');
            }
        }
