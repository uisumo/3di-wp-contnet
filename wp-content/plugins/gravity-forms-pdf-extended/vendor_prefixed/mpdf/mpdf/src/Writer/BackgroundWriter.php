        ...
        for ($i = 1; $i <= $patternCount; $i++) {
            ...
            if (\false !== \strpos($x_pos, '%')) {
                ...
                if (isset($bpa['w']) && $bpa['w']) {
                    $x_pos = $bpa['w'] * $x_pos - $orig_w / \GFPDF_Vendor\Mpdf\Mpdf::SCALE * $x_pos;
                } else {
                    $x_pos = $w * $x_pos - $orig_w / \GFPDF_Vendor\Mpdf\Mpdf::SCALE * $x_pos;
                }
            }
            ...
            if (\false !== \strpos($y_pos, '%')) {
                ...
                if (isset($bpa['h']) && $bpa['h']) {
                    $y_pos = $bpa['h'] * $y_pos - $orig_h / \GFPDF_Vendor\Mpdf\Mpdf::SCALE * $y_pos;
                } else {
                    $y_pos = $h * $y_pos - $orig_h / \GFPDF_Vendor\Mpdf\Mpdf::SCALE * $y_pos;
                }
            }
            if (isset($bpa['x']) && $bpa['x']) {
                $adj_x = ($x_pos + $bpa['x']) * \GFPDF_Vendor\Mpdf\Mpdf::SCALE;
            } else {
                $adj_x = ($x_pos + $x) * \GFPDF_Vendor\Mpdf\Mpdf::SCALE;
            }
            if (isset($bpa['y']) && $bpa['y']) {
                $adj_y = ($pgh - $y_pos - $bpa['y']) * \GFPDF_Vendor\Mpdf\Mpdf::SCALE - $orig_h;
            } else {
                $adj_y = ($pgh - $y_pos - $y) * \GFPDF_Vendor\Mpdf\Mpdf::SCALE - $orig_h;
            }
            $img_obj = \false;
            ...
            if (!$img_obj) {
                throw new \GFPDF_Vendor\Mpdf\MpdfException('Problem: Image object not found for background pattern ' . $img['i']);
            }
            $this->writer->object();
