...
class FontWriter
{
    ...
    public function __construct(\GFPDF_Vendor\Mpdf\Mpdf $mpdf, \GFPDF_Vendor\Mpdf\Writer\BaseWriter $writer, \GFPDF_Vendor\Mpdf\Fonts\FontCache $fontCache, $fontDescriptor)
    {
        $this->mpdf = $mpdf;
        $this->writer = $writer;
    ...
    public function writeFonts()
    {
                ...
                if ($used && !$asSubset) {
                    ...
                    if ($this->mpdf->repackageTTF || $this->mpdf->fonts[$fontkey]['TTCfontID'] > 0 || $this->mpdf->fonts[$fontkey]['useOTL'] > 0) {
                        ...
                        if ($this->fontCache->has($fontkey . '.ps.z') && $this->fontCache->jsonHas($fontkey . '.ps.json')) {
                            ...
                            $ttf = new \GFPDF_Vendor\Mpdf\TTFontFile($this->fontCache, $this->fontDescriptor);
        ...
        foreach ($this->mpdf->fonts as $k => $font) {
            ...
            if ($type === 'Type0') {
                ...
                $this->mpdf->fonts[$k]['n'] = $this->mpdf->n + 1;
                if ($this->mpdf->PDFA || $this->mpdf->PDFX) {
                    throw new \GFPDF_Vendor\Mpdf\MpdfException('Core fonts are not allowed in PDF/A1-b or PDFX/1-a files (Times, Helvetica, Courier etc.)');
                }
                $this->writer->object();
                $this->writer->write('<</Type /Font');
                ...
                if ($asSubset) {
                    $cidtogidmap = \str_pad('', 256 * 256 * 2, "\0");
                ...
                throw new \GFPDF_Vendor\Mpdf\MpdfException(\sprintf('Unsupported font type: %s (%s)', $type, $name));
            }
        }
    }
