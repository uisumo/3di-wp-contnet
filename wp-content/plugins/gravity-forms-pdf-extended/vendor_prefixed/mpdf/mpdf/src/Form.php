...
class Form
{
    ...
    public function __construct(\GFPDF_Vendor\Mpdf\Mpdf $mpdf, \GFPDF_Vendor\Mpdf\Otl $otl, \GFPDF_Vendor\Mpdf\Color\ColorConverter $colorConverter, \GFPDF_Vendor\Mpdf\Writer\BaseWriter $writer, \GFPDF_Vendor\Mpdf\Writer\FormWriter $formWriter)
    {
        $this->mpdf = $mpdf;
        $this->otl = $otl;
    ...
    function print_ob_imageinput($objattr, $w, $h, $texto, $rtlalign, $k, $blockdir, $is_table)
    {
        ...
        if ($this->mpdf->useActiveForms) {
            ...
            $this->writer->write(\sprintf('q %.3F 0 0 %.3F %.3F %.3F cm /I%d Do Q', $objattr['INNER-WIDTH'] * \GFPDF_Vendor\Mpdf\Mpdf::SCALE, $objattr['INNER-HEIGHT'] * \GFPDF_Vendor\Mpdf\Mpdf::SCALE, $objattr['INNER-X'] * \GFPDF_Vendor\Mpdf\Mpdf::SCALE, ($this->mpdf->h - ($objattr['INNER-Y'] + $objattr['INNER-HEIGHT'])) * \GFPDF_Vendor\Mpdf\Mpdf::SCALE, $objattr['ID']));
        ...
        if (isset($this->pdf_acro_array)) {
            ...
            foreach ($this->form_fonts as $fn) {
                if (\is_array($this->mpdf->fonts[$fn]['n'])) {
                    throw new \GFPDF_Vendor\Mpdf\MpdfException('Cannot use fonts with SMP or SIP characters for interactive Form elements');
                }
    ...
    function SetFormText($w, $h, $name, $value = '', $default = '', $title = '', $flags = [], $align = 'L', $hidden = \false, $maxlen = -1, $js = '', $background_col = \false, $border_col = \false)
    {
        ...
        if (!\preg_match('/^[a-zA-Z0-9_:\\-]+$/', $name)) {
            throw new \GFPDF_Vendor\Mpdf\MpdfException('Field [' . $name . '] must have a name attribute, which can only contain letters, numbers, colon(:), undersore(_) or hyphen(-)');
        }
    ...
    function SetFormButton($bb, $hh, $name, $value, $type, $title = '', $flags = [], $checked = \false, $disabled = \false, $background_col = \false, $border_col = \false, $noprint = \false)
    {
        ...
        if ($type === 'radio' || $type === 'checkbox') {
            if (!\preg_match('/^[a-zA-Z0-9_:\\-\\.]+$/', $value)) {
                throw new \GFPDF_Vendor\Mpdf\MpdfException("Field '" . $name . "' must have a value, which can only contain letters, numbers, colon(:), underscore(_), hyphen(-) or period(.)");
            }
        }
        if ($type === 'radio') {
    ...
    {
        $x *= \GFPDF_Vendor\Mpdf\Mpdf::SCALE;
        $y = $hPt - $y * \GFPDF_Vendor\Mpdf\Mpdf::SCALE;
        $x2 = $x + $w * \GFPDF_Vendor\Mpdf\Mpdf::SCALE;
        $y2 = $y - $h * \GFPDF_Vendor\Mpdf\Mpdf::SCALE;
    ...
    function _put_button_icon($array, $w, $h)
    {
        ...
        if (!$info) {
            throw new \GFPDF_Vendor\Mpdf\MpdfException('Cannot find Button image');
        }
        $this->writer->object();
        $this->writer->write('<<');
