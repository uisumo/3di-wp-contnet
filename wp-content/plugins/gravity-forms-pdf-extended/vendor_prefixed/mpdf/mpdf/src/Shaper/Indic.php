...
namespace GFPDF_Vendor\Mpdf\Shaper;
...
class Indic
{
    ...
    public static function set_indic_properties(&$info, $scriptblock)
    {
        ...
        if ($cat == self::OT_REPHA) {
             ...
             */
            if ($info['general_category'] == \GFPDF_Vendor\Mpdf\Ucdn::UNICODE_GENERAL_CATEGORY_NON_SPACING_MARK) {
                $cat = self::OT_N;
            }
        }
        /*
        ...
        if (self::FLAG($cat) & (self::FLAG(self::OT_C) | self::FLAG(self::OT_CM) | self::FLAG(self::OT_RA) | self::FLAG(self::OT_V) | self::FLAG(self::OT_NBSP) | self::FLAG(self::OT_DOTTEDCIRCLE))) {
            ...
            if ($scriptblock == \GFPDF_Vendor\Mpdf\Ucdn::SCRIPT_KHMER) {
    ...
    public static function set_syllables(&$o, $s, &$broken_syllables)
    {
        ...
        while ($ptr < \strlen($s)) {
            ...
            } elseif (($ptr == 0 || $o[$ptr - 1]['general_category'] < \GFPDF_Vendor\Mpdf\Ucdn::UNICODE_GENERAL_CATEGORY_LOWERCASE_LETTER || $o[$ptr - 1]['general_category'] > \GFPDF_Vendor\Mpdf\Ucdn::UNICODE_GENERAL_CATEGORY_NON_SPACING_MARK) && \preg_match('/^(RH|r)?[sD][N]?([ZJ]?H[CR]m*)?([M]*[N]?[H]?)?[S]?[v]{0,2}/', \substr($s, $ptr), $ma)) {
                // From HarfBuzz:
    ...
    public static function insert_dotted_circles(&$info, $dottedcircle)
    {
        ...
        if (isset($info[$idx])) {
            throw new \GFPDF_Vendor\Mpdf\MpdfException('Unexpected error occurred in Indic processing');
        }
    ...
    public static function initial_reordering_syllable(&$info, $GSUBdata, $indic_config, $scriptblock, $is_old_spec, $start, $end)
    {
        ...
        */
        ...
        if ($is_old_spec && $scriptblock == \GFPDF_Vendor\Mpdf\Ucdn::SCRIPT_DEVANAGARI) {
    ...
    public static function final_reordering_syllable(&$info, $GSUBdata, $indic_config, $scriptblock, $is_old_spec, $start, $end)
    {
        ...
        if ($start + 1 < $end && $start < $base) {
             ...
             */
            if ($scriptblock != \GFPDF_Vendor\Mpdf\Ucdn::SCRIPT_MALAYALAM && $scriptblock != \GFPDF_Vendor\Mpdf\Ucdn::SCRIPT_TAMIL) {
        ...
        if ($info[$start]['indic_position'] == self::POS_PRE_M && ($start == 0 || ($info[$start - 1]['general_category'] < \GFPDF_Vendor\Mpdf\Ucdn::UNICODE_GENERAL_CATEGORY_FORMAT || $info[$start - 1]['general_category'] > \GFPDF_Vendor\Mpdf\Ucdn::UNICODE_GENERAL_CATEGORY_NON_SPACING_MARK))) {
