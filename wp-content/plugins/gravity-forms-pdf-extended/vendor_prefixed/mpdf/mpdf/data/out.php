...
if (\strstr($tempfilename, '/') || \strstr($tempfilename, '\\')) {
    throw new \GFPDF_Vendor\MpdfException('Output filename can not not contain \\ or /');
}
...
if ($tempfilename && \file_exists($path . $tempfilename)) {
    ...
    if ($dest === 'I') {
        ...
        if (\headers_sent()) {
            throw new \GFPDF_Vendor\MpdfException('Some data has already been output to browser, can\'t send PDF file');
        }
