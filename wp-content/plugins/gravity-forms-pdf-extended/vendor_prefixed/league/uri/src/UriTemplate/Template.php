...
final class Template
{
    ...
    private function __construct(string $template, \GFPDF_Vendor\League\Uri\UriTemplate\Expression ...$expressions)
    {
    ...
    public static function createFromString($template) : self
    {
        ...
        if (\false !== \strpos($remainder, '{') || \false !== \strpos($remainder, '}')) {
            throw new \GFPDF_Vendor\League\Uri\Exceptions\SyntaxError('The template "' . $template . '" contains invalid expressions.');
        }
        $names = [];
        ...
        foreach ($findings as $finding) {
            if (!isset($names[$finding[0]])) {
                $arguments[] = \GFPDF_Vendor\League\Uri\UriTemplate\Expression::createFromString($finding[0]);
                $names[$finding[0]] = 1;
            }
        }
