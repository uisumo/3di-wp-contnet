...
class QueryPathEventHandler implements \GFPDF_Vendor\QueryPath\CSS\EventHandler, \GFPDF_Vendor\QueryPath\CSS\Traverser
{
     ...
     */
    public function find($filter) : \GFPDF_Vendor\QueryPath\CSS\QueryPathEventHandler
    {
    ...
    public function attributeNS($lname, $ns, $value = NULL, $operation = \GFPDF_Vendor\QueryPath\CSS\EventHandler::IS_EXACTLY)
    {
        $matches = $this->candidateList();
        $found = new \SplObjectStorage();
        if (\count($matches) == 0) {
    ...
    {
        // New in Quark.
        $this->nthChild($groupSize, $elementInGroup, \true);
    }
    /**
    ...
    */
    ...
    {
        $this->nthOfTypeChild($groupSize, $elementInGroup, \true);
    }
    /**
    ...
    protected function attrValMatches($needle, $haystack, $operation)
    {
        ...
        switch ($operation) {
            case \GFPDF_Vendor\QueryPath\CSS\EventHandler::IS_EXACTLY:
                return $needle == $haystack;
            case \GFPDF_Vendor\QueryPath\CSS\EventHandler::CONTAINS_WITH_SPACE:
                return \in_array($needle, \explode(' ', $haystack));
            case \GFPDF_Vendor\QueryPath\CSS\EventHandler::CONTAINS_WITH_HYPHEN:
                return \in_array($needle, \explode('-', $haystack));
            case \GFPDF_Vendor\QueryPath\CSS\EventHandler::CONTAINS_IN_STRING:
                return \strpos($haystack, $needle) !== \false;
            case \GFPDF_Vendor\QueryPath\CSS\EventHandler::BEGINS_WITH:
                return \strpos($haystack, $needle) === 0;
            case \GFPDF_Vendor\QueryPath\CSS\EventHandler::ENDS_WITH:
