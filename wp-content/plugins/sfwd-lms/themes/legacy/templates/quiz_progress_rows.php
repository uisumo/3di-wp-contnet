	...
	<?php if ( ! empty( $quiz_title ) ) : ?>
		<p id="ld-quiz-<?php echo $v['time']; ?>">
			...
			if ( ( $user_id == get_current_user_id() ) || ( learndash_is_admin_user() ) || ( learndash_is_group_leader_user() ) ) {
				...
				if ( ( isset( $v['statistic_ref_id'] ) ) && ( ! empty( $v['statistic_ref_id'] ) ) ) {
					...
					if ( apply_filters(
						...
						?>
							<a class="user_statistic" data-statistic_nonce="<?php echo wp_create_nonce( 'statistic_nonce_' . $v['statistic_ref_id'] . '_' . get_current_user_id() . '_' . $user_id ); ?>" data-user_id="<?php echo $user_id; ?>" data-quiz_id="<?php echo $v['pro_quizid']; ?>" data-ref_id="<?php echo intval( $v['statistic_ref_id'] ); ?>" href="#"><?php _e( 'Statistics', 'learndash' ); ?></a>
							<?php
					}
				}
			}
			...
			if ( current_user_can( 'wpProQuiz_edit_quiz' ) ) {
				...
				<a class="remove-quiz" data-quiz-user-id="<?php echo $user_id; ?>" data-quiz-nonce="<?php echo wp_create_nonce( 'remove_quiz_' . $user_id . '_' . $v['quiz'] . '_' . $v['time'] ); ?>" href="#" title="
																	<?php
																		// translators: placeholder: quiz.
																		echo sprintf( _x( 'remove this %s item', 'placeholder: quiz', 'learndash' ), learndash_get_custom_label_lower( 'quiz' ) )
																	?>
					"><?php echo _x( '(remove)', 'profile remove quiz link label', 'learndash' ); ?></a>
					<?php
				}
			}
			?>
