...
if ( ! class_exists( 'LDLMS_Course_Steps' ) ) {
	...
	class LDLMS_Course_Steps {
		...
		protected function set_step_to_course_legacy() {
			...
			$sql_str          = $wpdb->prepare(
				'SELECT posts.ID as post_id FROM ' . $wpdb->posts . ' as posts INNER JOIN ' . $wpdb->postmeta . ' as postmeta ON posts.ID = postmeta.post_id WHERE 1=1 AND posts.post_type IN (' . "'" . implode( "','", $this->steps_post_types ) . "'" . ') AND postmeta.meta_key = %s AND postmeta.meta_value = %d',
				'course_id',
				$this->course_id
			);
			$course_steps_old = $wpdb->get_col( $sql_str );
		...
		public function set_step_to_course_relationship( $steps = array(), $parent_lesson_id = 0 ) {
			global $wpdb;
			...
			if ( ! empty( $steps ) ) {
				foreach ( $steps as $steps_type => $steps_type_set ) {
					...
					} elseif ( ( 'sfwd-quiz' === $steps_type ) && ( 0 === $parent_lesson_id ) ) {
		...
		public function set_step_to_course_order( $steps = array() ) {
			global $wpdb;
		...
		public function load_steps_legacy() {
		...
		public function get_children_steps( $post_id = 0, $post_type = '' ) {
		...
		public static function steps_split_keys( $steps, $parent_type = '' ) {
			if ( 'sfwd-lessons' === $parent_type ) {
				$course_steps_split_keys = array(
					'sfwd-topic' => array(),
					'sfwd-quiz'  => array(),
				);
			} elseif ( 'sfwd-topic' === $parent_type ) {
				$course_steps_split_keys = array(
					'sfwd-quiz' => array(),
				);
			} elseif ( 'sfwd-quiz' === $parent_type ) {
				$course_steps_split_keys = array();
