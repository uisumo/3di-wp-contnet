...
foreach ($allAssets as $obj) {
	...
	*/
	...
	$isLoadException = $isLoadExceptionPerPage || $isLoadExceptionForCurrentPostType || $isLoadExceptionRegExMatch;
	...
	if (strpos($row['class'], 'wpacu_not_load') === false && isset($data['current_unloaded_assets']['css']) && in_array($row['obj']->handle, $data['current_unloaded_assets']['css'])) {
		$row['class'] .= ' wpacu_not_load';
	}
...
foreach ($allAssetsFinal as $assetHandle => $row) {
	...
	if (isset($data['rows_build_array']) && $data['rows_build_array']) {
		...
		if (isset($data['rows_by_location']) && $data['rows_by_location']) {
			...
			if (isset($data['current_unloaded_assets']['css']) && in_array($row['obj']->handle, $data['current_unloaded_assets']['css'])) {
