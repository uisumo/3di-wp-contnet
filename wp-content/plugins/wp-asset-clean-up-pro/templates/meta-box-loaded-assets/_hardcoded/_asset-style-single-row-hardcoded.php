...
if (strpos($data['row']['class'], 'wpacu_not_load') === false && isset($data['current_unloaded_assets']['css']) && in_array($data['row']['obj']->handle, $data['current_unloaded_assets']['css'])) {
    ...
    <td style="position: relative;" data-wpacu-row-status="<?php echo esc_attr($dataRowStatusAttr); ?>">
        ...
        <div class="wpacu_handle_row_expanded_area <?php if ($rowIsContracted) { echo 'wpacu_hide'; } ?>">
	        <div class="wrap_bulk_unload_options">
		        ...
		        if ($data['post_type'] && $data['post_type'] !== 'attachment' && ! $data['row']['is_post_type_unloaded'] && ! empty($data['post_type_has_tax_assoc'])) {
			        // Unload on all pages where this [post] post type has a certain taxonomy set for it (e.g. a Tag or a Category) (if applicable)
			        include '_asset-style-single-row-hardcoded/_unload-post-type-taxonomy.php';
		        }
