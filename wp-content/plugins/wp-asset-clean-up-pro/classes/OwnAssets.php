...
HTML;
		...
		// Load jQuery Chosen on "Settings", "CSS & JS Manager" -> "Manage CSS/JS" (any post type page)
		$isManageCssPostPage = ($page === WPACU_PLUGIN_ID . '_assets_manager')
            && isset($_GET['wpacu_for'])
            && in_array($_GET['wpacu_for'], array('pages', 'posts', 'custom-post-types', 'media-attachment'));

		if ( $page === WPACU_PLUGIN_ID . '_settings' || $isManageCssPostPage ) {
		    $this->loadjQueryChosen();
        }
