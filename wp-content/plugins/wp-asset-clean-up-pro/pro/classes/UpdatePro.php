...
class UpdatePro
{
	...
	public static function updateHandleMediaQueriesLoad()
	{
		...
		if (isset($bucketToUse['styles']) && ! empty($bucketToUse['styles'])) {
			foreach ($bucketToUse['styles'] as $styleHandle => $styleMediaQueryArray) {
		...
		if (isset($bucketToUse['scripts']) && ! empty($bucketToUse['scripts'])) {
			foreach ( $bucketToUse['scripts'] as $scriptHandle => $scriptMediaQueryArray ) {
				$mediaQueryLoadEnable = isset( $scriptMediaQueryArray['enable'] ) && $scriptMediaQueryArray['enable'];
				$mediaQueryLoadValue  = isset( $scriptMediaQueryArray['value'] ) ? stripslashes( trim($scriptMediaQueryArray['value']) ) : '';
