...
class GoogleMapsProvider extends AbstractProvider implements LocaleAwareProviderInterface
{
    ...
    protected function executeQuery($query)
    {
        ...
        if ('REQUEST_DENIED' === $json->status && strpos($json->error_message, 'You must use an API key to authenticate') !== false ) {
            throw new NoResultException(sprintf('Could not execute query: Empty %s API key', $this->getName()));
        }
        ...
        if ('REQUEST_DENIED' === $json->status && 'This API key is not authorized to use this service or API.' === $json->error_message) {
            throw new InvalidCredentialsException(sprintf( 'API key is not authorized to use this service or API'));
        }

		if ('REQUEST_DENIED' === $json->status && 'This API project is not authorized to use this API.' === $json->error_message) {
			throw new InvalidCredentialsException(sprintf( 'This API project is not authorized to use this API'));
		}
