<?php
/**
 * GravityView Maps Extension - GF Entry meta box class
 *
...
class GravityView_Maps_GF_Entry_Geocoding extends GravityView_Maps_Component {
	...
	public function get_geocoding_form_html( GF_Field_Address $field, $entry = array(), $form = array() ) {
		...
		$gv_maps_cache_markers = $GLOBALS['gravityview_maps']->component_instances['cache-markers'];
		...
		if ( $has_background_process ) {
			$position_string = '<span class="description">' . esc_html__( 'In progress.', 'gravityview-maps' ) . '</span>';
		} elseif ( empty( $lat ) || empty( $long ) ) {
			$has_geocoding   = false;
			$position_string = esc_html__( 'Not yet geocoded.', 'gravityview-maps' );
		} else {
			$has_geocoding   = true;
			$position_string = $lat . ', ' . $long;
		}

		$output .= sprintf( esc_html__( 'Geocoding: %s', 'gravityview-maps' ), '<span class="gv-maps-geocoding-position" aria-live="polite">' . $position_string . '</span>' );
...
HTML;
	...
	protected static function strings() {
		...
		return array(
			'field_label'                    => esc_html_x( '%s (ID #%d)', 'Used to identify address field: %s is replaced with field label & %d is replaced with field ID', 'gravityview-maps' ),
			'lat_label'                      => esc_html__( 'Latitude', 'gravityview-maps' ),
			'long_label'                     => esc_html__( 'Longitude', 'gravityview-maps' ),
			'update_label'                   => esc_html_x( 'Update', 'Update latitude/longitude coordinates', 'gravityview-maps' ),
			'updated_success_notice'         => esc_html__( 'Coordinates were successfully updated.', 'gravityview-maps' ),
			'saved_success_notice'           => esc_html__( 'Coordinates were successfully saved.', 'gravityview-maps' ),
			'cleared_success_notice'         => esc_html__( 'Coordinates were successfully cleared.', 'gravityview-maps' ),
			'updated_error_notice'           => esc_html__( 'Unable to update coordinates.', 'gravityview-maps' ),
			'saved_error_notice'             => esc_html__( 'Unable to save coordinates.', 'gravityview-maps' ),
			'geocode_label'                  => esc_html_x( 'Geocode This Address', 'Get latitude/longitude from address', 'gravityview-maps' ),
			'save_coordinates_label'         => esc_html__( 'Save Latitude/Longitude', 'gravityview-maps' ),
			'geocode_refresh_label'          => esc_html_x( 'Refresh Geocoding', 'Get latitude/longitude from address', 'gravityview-maps' ),
			'geocode_refresh_title'          => esc_attr__( 'Fetch the longitude and latitude for this address.', 'gravityview-maps' ),
			'geocoded_success_notice'        => esc_html__( 'Coordinates were fetched and saved for this address.', 'gravityview-maps' ),
			'geocoded_error_notice'          => esc_html__( 'Unable to retrieve coordinates for this address.', 'gravityview-maps' ),
			'remote_request_fail_notice'     => esc_html__( 'Unable to perform this action. Please try again or contact support.', 'gravityview-maps' ),
			'geocoding_not_available_notice' => esc_html__( 'This address has not yet been geocoded.', 'gravityview-maps' ),
			'geocoding_not_yet'              => esc_html__( 'Not yet geocoded.', 'gravityview-maps' ),
			'open_google_maps_notice'        => esc_html__( 'See on Google Maps', 'gravityview-maps' ),
		);
	}
	...
	public function add_entry_meta( $entry_meta, $form_id ) {
		...
		$address_fields = GFAPI::get_fields_by_type( GFAPI::get_form( $form_id ), array( 'address' ) );
		...
		foreach ( $address_fields as $address_field ) {
			$entry_meta[ 'gvmaps_lat_' . $address_field->id ]  = array(
				'label'             => sprintf( esc_html_x( '%s Latitude', '%s is replaced by the field label of an address field', 'gravityview_maps', 'gravityview-maps' ), esc_html( $address_field->label ) ),
				'is_numeric'        => true,
				'is_default_column' => false
			);
			$entry_meta[ 'gvmaps_long_' . $address_field->id ] = array(
				'label'             => sprintf( esc_html_x( '%s Longitude', '%s is replaced by the field label of an address field', 'gravityview_maps', 'gravityview-maps' ), esc_html( $address_field->label ) ),
				'is_numeric'        => true,
				'is_default_column' => false
			);
		}
	...
	public function geocode_address() {
		...
		if ( ! GravityView_Roles_Capabilities::has_cap( $this->edit_caps, $entry_id ) ) {
			wp_send_json_error( new WP_Error( 'no_access', esc_html__( 'You do not have permission to edit this entry.', 'gravityview-maps' ) ), false, array( 'response' => 400 ) );
		}
	...
	public function save_latlong() {
		...
		/** @var GravityView_Maps_Cache_Markers $gv_maps_cache_markers */
		$gv_maps_cache_markers = $GLOBALS['gravityview_maps']->component_instances['cache-markers'];
		...
		if ( empty( $lat ) || empty( $long ) ) {
			wp_send_json_error( new WP_Error( 'invalid_coordinates', esc_html__( 'The coordinates are not formatted properly.', 'gravityview-maps' ) ), false, array( 'response' => 400 ) );
		}

		if ( $lat > 90 || $lat < - 90 ) {
			wp_send_json_error( new WP_Error( 'invalid_coordinates', esc_html__( 'The latitude is out of bounds.', 'gravityview-maps' ) ), false, array( 'response' => 400 ) );
		}

		if ( $long > 180 || $long < - 180 ) {
			wp_send_json_error( new WP_Error( 'invalid_coordinates', esc_html__( 'The longitude is out of bounds.', 'gravityview-maps' ) ), false, array( 'response' => 400 ) );
		}
	...
	public function enqueue_scripts() {
		...
		wp_enqueue_style( self::ASSETS_HANDLE, plugins_url( 'assets/css/gv-maps-gf-entry-geocoding.css', $this->loader->_path ), array(), $this->loader->plugin_version );

		wp_enqueue_script( self::ASSETS_HANDLE, plugins_url( 'assets/js/gv-maps-gf-entry-geocoding' . $script_debug . '.js', $this->loader->_path ), array( 'jquery' ), $this->loader->plugin_version, true );
