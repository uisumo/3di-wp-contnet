...
class ewdupcpViewCatalog extends ewdupcpView {
	...
	public function filter_products( $product_posts ) {
		...
		foreach ( $product_posts as $product_post ) {
			...
			if ( ! empty( $this->filtering_text ) ) {
				...
				if ( stripos( $product->name, $this->filtering_text ) !== false ) { 

					$match = true; 
				}
				elseif ( in_array( 'description', $ewd_upcp_controller->settings->get_setting( 'product-search' ) ) and stripos( $product->description, $this->filtering_text ) !== false ) { 
				...
				elseif ( in_array( 'custom_fields', $ewd_upcp_controller->settings->get_setting( 'product-search' ) ) ) {
					...
					foreach ( $product->custom_fields as $field_id => $field_values ) {
						...
						foreach ( (array) $field_values as $field_value ) {
							...
							if ( stripos( $field_value, $this->filtering_text ) !== false ) { $match = true; }
						}
					}
				}
	...
	public function set_request_parameters() {
		...
		if ( ! empty( $_GET['prod_name'] ) ) { $this->filtering_text = sanitize_text_field( $_GET['prod_name'] ); }
