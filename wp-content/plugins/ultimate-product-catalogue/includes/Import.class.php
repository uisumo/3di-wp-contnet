	...
	public function import_products() {
	    ...
	    foreach ( $data as $product_data ) {
	        ...
	        if ( $post_id ) {
	        	...
	        	if ( ! empty( $post_meta['price'] ) ) { update_post_meta( $post_id, 'price', $post_meta['price'] ); }
	        	if ( ! empty( $post_meta['sale_price'] ) ) { update_post_meta( $post_id, 'sale_price', $post_meta['sale_price'] ); }
	        	if ( ! empty( $post_meta['link'] ) ) { update_post_meta( $post_id, 'link', $post_meta['link'] ); }
	        	if ( ! empty( $post_meta['display'] ) ) { update_post_meta( $post_id, 'display', $post_meta['display'] ); }
	        	...
	        	if ( ! empty( $post_meta['category'] ) or ! empty( $post_meta['subcategory'] ) ) { 
	        		...
	        		wp_set_post_terms( $post_id, $category_ids, EWD_UPCP_PRODUCT_CATEGORY_TAXONOMY, true );
	        	}
