...
<?php if ( @$lesson_progression_enabled && ! @$previous_lesson_completed ) : ?>
	...
	if ( ( ! empty( $previous_item ) ) && ( $previous_item instanceof WP_Post ) ) {
		if ( $previous_item->post_type == 'sfwd-quiz' ) {
			echo sprintf( esc_attr_x( 'Please go back and complete the previous <a class="learndash-link-previous-incomplete" href="%s">%s</a>.', 'placeholders: quiz URL, quiz label', 'learndash' ), get_permalink( $previous_item->ID ), LearnDash_Custom_Label::label_to_lower( 'quiz' ) );

		} else if ( $previous_item->post_type == 'sfwd-topic' ) {
			echo sprintf( esc_attr_x( 'Please go back and complete the previous <a class="learndash-link-previous-incomplete" href="%s">%s</a>.', 'placeholders: topic URL, topic label', 'learndash' ), get_permalink( $previous_item->ID ), LearnDash_Custom_Label::label_to_lower( 'topic' ) );
		} else {
			echo sprintf( esc_attr_x( 'Please go back and complete the previous <a class="learndash-link-previous-incomplete" href="%s">%s</a>.', 'placeholders: lesson URL, lesson label', 'learndash' ), get_permalink( $previous_item->ID ), LearnDash_Custom_Label::label_to_lower( 'lesson' ) );
		}
	...
	<?php if ( ! empty( $topics ) ) : ?>
		...
		<!--<div id="learndash_lesson_topics_list" class="learndash_lesson_topics_list">
			<div id='learndash_topic_dots-<?php /*echo esc_attr( $post->ID ); */ ?>' class="learndash_topic_dots type-list">
				<strong><?php /*printf( esc_attr_x( '%s %s', 'Lesson Topics Label', 'learndash'), LearnDash_Custom_Label::get_label( 'lesson' ), LearnDash_Custom_Label::get_label( 'topics' ) ); */ ?></strong>
				<ul>
					<?php /*$odd_class = ''; */ ?>
