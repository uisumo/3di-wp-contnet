			...
			<?php endif; ?>
                    ...
                    <div id="lessons_list" class="lessons_list">
						...
						<?php foreach ( $lessons as $lesson ) :
                            ...
                            <div class='post-<?php echo esc_attr( $lesson['post']->ID ); ?> <?php echo esc_attr( $lesson['sample'] ); ?>'>
                                ...
                                <h4>
                                    <a class='<?php echo $is_not_available ? 'notavailable' : esc_attr( $lesson['status'] ); ?>'
                                       href='<?php echo esc_attr( $lesson['permalink'] ); ?>'><?php echo $lesson['post']->post_title; ?></a>
									...
									<small class="notavailable_message">
										<?php echo sprintf( esc_attr__( 'CCC Available on: %s ', 'learndash' ), learndash_adjust_date_time_display( $lesson['lesson_access_from'] ) ); ?>
									...
									<?php if ( ! empty( $topics ) ) : ?>
                                        <div id='learndash_topic_dots-<?php echo esc_attr( $lesson['post']->ID ); ?>'
				...
				if ( isset( $lessons_results['pager'] ) ) {
					...
					if ( $lessons_results['pager']['paged'] < $lessons_results['pager']['total_pages'] ) {
						echo '<div class="alignright"><a href="' . add_query_arg( 'ld-lesson-page', intval( $lessons_results['pager']['paged'] ) + 1 ) . '">' . esc_attr__( 'Next »', 'learndash' ) . '</a></div>';
					}
					echo '</div>';
