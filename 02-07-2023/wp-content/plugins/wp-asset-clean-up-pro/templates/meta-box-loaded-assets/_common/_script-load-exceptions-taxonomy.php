...
$handleLoadViaTax = ( isset( $data['handle_load_via_tax']['scripts'][ $data['row']['obj']->handle ] ) && $data['handle_load_via_tax']['scripts'][ $data['row']['obj']->handle ] )
	? $data['handle_load_via_tax']['scripts'][ $data['row']['obj']->handle ]
	: false;
...
<!-- [wpacu_pro] -->
<li>
    <label for="wpacu_load_it_via_tax_option_script_<?php echo htmlentities(esc_attr($data['row']['obj']->handle), ENT_QUOTES); ?>">
        <input data-handle="<?php echo htmlentities(esc_attr($data['row']['obj']->handle), ENT_QUOTES); ?>"
               data-handle-for="script"
               id="wpacu_load_it_via_tax_option_script_<?php echo htmlentities(esc_attr($data['row']['obj']->handle), ENT_QUOTES); ?>"
               class="wpacu_load_it_via_tax_checkbox wpacu_load_exception wpacu_load_rule_input wpacu_bulk_load"
               type="checkbox"
               name="<?php echo WPACU_FORM_ASSETS_POST_KEY; ?>[scripts][load_it_post_type_via_tax][<?php echo htmlentities(esc_attr($data['row']['obj']->handle), ENT_QUOTES); ?>][enable]"
    ...
    <div class="wpacu_handle_manage_via_tax_input_wrap wpacu_handle_load_via_tax_input_wrap <?php if ( ! $isLoadViaTaxEnabledWithValues ) { echo 'wpacu_hide'; } ?>">
        <div class="wpacu_manage_via_tax_rule_area" style="min-width: 300px;">
            <select name="<?php echo WPACU_FORM_ASSETS_POST_KEY; ?>[scripts][load_it_post_type_via_tax][<?php echo htmlentities(esc_attr($data['row']['obj']->handle), ENT_QUOTES); ?>][values][]"
                    ...
                    data-handle-for="script"><?php if ( $isLoadViaTaxEnabledWithValues ) { echo \WpAssetCleanUpPro\MainPro::loadDDOptionsForAllSetTermsForPostType($data['post_type'], 'scripts', $data['row']['obj']->handle, $handleLoadViaTax['values'], 'load_exception'); } ?></select>
        </div>
    </div>
	<?php
