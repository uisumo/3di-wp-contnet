        ...
        if ( $for === 'default' ) {
            ...
            if (isset($handleData['script_attrs']['term']) && ! empty($handleData['script_attrs']['term'])) {
	            ...
	            foreach ($handleData['script_attrs']['term'] as $termId => $attrList) {
		            $taxData = term_exists( $termId ) ? get_term( $termId ) : false;
				        ...
				        foreach ( $handleData['attrs_no_load'] as $attrSetIn => $attrSetValues ) {
					        ...
					        if ( $attrSetIn === 'post' ) {
						        ...
						        foreach ( $attrSetValues as $postId => $attrSetValuesTwo ) {
							        ...
							        $postPagesList .= '<a title="Post Title: '.esc_attr($postTitle).', Post Type: '.$postType.'" class="wpacu-tooltip" target="_blank" href="'.esc_url(admin_url('post.php?post='.$postId.'&action=edit')).'">'.$postId.'</a> | ';
						        }
            ...
            if ( $src ) {
	            ...
	            if ($maybeInactiveAsset = \WpAssetCleanUp\Misc::maybeIsInactiveAsset($src)) {
	                ...
	                ?>
                    <div>
                        <small><strong>Note:</strong> <span style="color: darkred;">The plugin `<strong><?php echo esc_html($maybeInactiveAsset); ?></strong>` seems to be inactive, thus any rules set are also inactive &amp; irrelevant, unless you re-activate the plugin.</span></small>
                        <form method="post" action="" style="display: inline-block;">
                            ...
                            <script type="text/javascript">
                                var wpacuClearAllRulesConfirmMsg = '<?php echo esc_js($clearAllRulesConfirmMsg); ?>';
                            </script>
	...
	public static function renderHandleChangesOutput($handleData)
	{
			...
			if (isset($handleData['unload_bulk']['post_type_via_tax'])) {
			    foreach ($handleData['unload_bulk']['post_type_via_tax'] as $postType => $termIds) {
				    ...
				    if ( ! empty($taxLabelsToNames) ) {
					    $handleChangesOutput['bulk'] .= ' Unloaded on all pages of <strong>' . $postType . '</strong> post type with these taxonomies: ';
		...
		if (isset($handleData['unload_on_this_page']['post'])) {
			...
			foreach ($handleData['unload_on_this_page']['post'] as $postId) {
				...
				$postTitle  = $postData->post_title;
				$postType   = $postData->post_type;
				$postsList .= '<a title="Post Title: '.$postTitle.', Post Type: '.$postType.'" class="wpacu-tooltip" target="_blank" href="'.esc_url(admin_url('post.php?post='.$postId.'&action=edit')).'">'.$postId.'</a>, ';
			}
