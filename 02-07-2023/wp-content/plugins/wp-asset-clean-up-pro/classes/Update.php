...
HTML;
	...
	public function saveLoadExceptionsPostType($wpacuPostType)
    {
        ...
        if (isset($_POST[$referenceKey]['styles']) || isset($_POST[$referenceKey]['scripts'])) {
	        if ( isset( $_POST[$referenceKey]['styles']) && ! empty( $_POST[$referenceKey]['styles'] ) ) {
	            foreach ($_POST[$referenceKey]['styles'] as $assetHandle => $values) {
                    $assetValue = isset($values['load_it_post_type']) && $values['load_it_post_type'] ? $values['load_it_post_type'] : ''; // '1' or ''
                    $loadExceptionsStyles[$assetHandle] = $assetValue;
	            }
	        }

	        if ( isset( $_POST[$referenceKey]['scripts']) && ! empty( $_POST[$referenceKey]['scripts'] ) ) {
		        foreach ($_POST[$referenceKey]['scripts'] as $assetHandle => $values) {
			        $assetValue = isset($values['load_it_post_type']) && $values['load_it_post_type'] ? $values['load_it_post_type'] : ''; // '1' or ''
			        $loadExceptionsScripts[$assetHandle] = $assetValue;
		        }
	        }
        } else {
	        // Prior to v1.2.0.5
	...
	public static function updateIgnoreChild($mainVarToUse = array())
	{
		...
		if (isset($bucketToUse['styles']) && ! empty($bucketToUse['styles'])) {
			foreach ($bucketToUse['styles'] as $styleHandle => $styleVal) {
		...
		if (isset($bucketToUse['scripts']) && ! empty($bucketToUse['scripts'])) {
			foreach ($bucketToUse['scripts'] as $scriptHandle => $scriptVal) {
