 ...
 * @since 2.4.7
 *
 * @returns array An array of MySQL privilege grants.
 */
...
function learndash_is_protected_meta( $protected = false, $meta_key = '', $meta_type = '' ) {
	if ( ( 'post' === $meta_type ) && ( ! empty( $meta_key ) ) && ( '_' !== $meta_key[0] ) ) {
		...
		// If post type is not empty and onf other LD types.
...
function learndash_purchase_invoice_filepath( $post_id = 0 ) {
	...
	if ( ( $ld_upload_dir ) && ( ! empty( $ld_upload_dir ) ) && ( file_exists( $ld_upload_dir ) ) && ( is_writable( $ld_upload_dir ) ) ) {
		 ...
		 * @param string $purchase_invoices_dir Directory location to save purchase invoice.
		 */
