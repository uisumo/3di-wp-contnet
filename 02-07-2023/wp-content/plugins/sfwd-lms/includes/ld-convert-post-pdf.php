...
if ( ! function_exists( 'learndash_purchase_invoice_pdf' ) ) {
	 ...
	 *
	 * @return array Generated filename and path.
	 */
	function learndash_purchase_invoice_pdf( $pdf_content = array() ) {
		...
		$transaction_user_id = get_post_meta( $transaction_id, 'user_id', true );
		...
		$purchased_post = get_post_meta( $transaction_id, 'post_id', true );
		 ...
		 */
		$config_lang_tmp = '';
		if ( ( isset( $_GET['pdf_lang'] ) ) && ( ! empty( $_GET['pdf_lang'] ) ) ) { // phpcs:ignore WordPress.Security.NonceVerification.Recommended
			$config_lang_tmp = substr( esc_attr( sanitize_text_field( wp_unslash( $_GET['pdf_lang'] ) ) ), 0, 3 ); // phpcs:ignore WordPress.Security.NonceVerification.Recommended
		} elseif ( ( isset( $_GET['lang'] ) ) && ( ! empty( $_GET['lang'] ) ) ) { // phpcs:ignore WordPress.Security.NonceVerification.Recommended
			$config_lang_tmp = substr( esc_attr( sanitize_text_field( wp_unslash( $_GET['lang'] ) ) ), 0, 3 ); // phpcs:ignore WordPress.Security.NonceVerification.Recommended
		}
		 ...
		 */
		$learndash_pdf_username = apply_filters( 'learndash_purchase_invoice_pdf_username', $purchase_invoice_args['user']->user_login, $transaction_user_id, $purchase_invoice_args['purchase_invoice_id'] );
		...
		$transaction_payment_meta = learndash_transaction_get_payment_meta( $transaction_id );
		...
		// get featured image.
		$img_file = $pdf_content['company_logo']['value'];
		...
		if ( '' !== $img_file ) {
			...
			if ( 'right' === $pdf_content['logo_location']['value'] ) {
				$logo_position = 'R';
			} else {
				$logo_position = 'L';
			}
		...
		$filepath = learndash_purchase_invoice_filepath( $purchased_post );
		$filename = learndash_purchase_invoice_filename( $transaction_user_id, $purchased_post );
		$file     = array(
			'filepath' => $filepath,
			'filename' => $filename,
		);
