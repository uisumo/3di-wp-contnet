...
if ( ! class_exists( 'Learndash_Site_Health' ) ) {
	...
	class Learndash_Site_Health {
		...
		private function map_settings_fields(): array {
			...
			if ( ! class_exists( 'LearnDash_Razorpay_Integration' ) ) {
			...
			$paypal_configured     = 'on' === LearnDash_Settings_Section::get_section_setting( 'LearnDash_Settings_Section_PayPal', 'enabled' ) && ! empty( LearnDash_Settings_Section::get_section_setting( 'LearnDash_Settings_Section_PayPal', 'paypal_email' ) ); // Should be refactored in the transactions and gateways branch to use the PayPal gateway class like other gateways.
			$stripe_configured     = ( new LearnDash_Stripe_Connect_Checkout_Integration() )->is_ready();
			$razorpay_configured   = ( new LearnDash_Razorpay_Integration() )->is_ready();
