 ...
 *
 * Webhook payload examples can be found here https://razorpay.com/docs/webhooks/payloads/subscriptions.
 *
 * @since 4.2.0
 *
 * @package LearnDash
...
if ( ! class_exists( 'LearnDash_Razorpay_Integration' ) && class_exists( 'LearnDash_Payment_Gateway_Integration' ) ) {
	...
	class LearnDash_Razorpay_Integration extends LearnDash_Payment_Gateway_Integration {
		...
		protected function is_test_mode(): bool {
			return isset( $this->settings['test_mode'] ) && 1 === (int) $this->settings['test_mode'];
		}
		...
		public function process_webhook(): void {
			...
			if ( ! apply_filters( 'learndash_process_webhook_razorpay', true, $event ) ) {
				wp_send_json_success(
					array( 'message' => 'Processing bypassed by the "learndash_process_webhook_razorpay" filter.' ),
					200
				);
			}

			try {
				$this->api->utility->verifyWebhookSignature( // @phpstan-ignore-line -- magic method.
					$input,
			...
			if ( is_null( $this->post ) ) {
				wp_send_json_error(
					new WP_Error(
						'bad_request',
						sprintf(
							'Post validation failed. The post with the ID %d was not found.',
							(int) $entity['notes']['post_id']
						)
					),
					422
				);
			...
			if ( is_null( $this->user ) ) {
				wp_send_json_error(
					new WP_Error( 'bad_request', 'User validation failed. The user was not found or had not been able to be created successfully.' ),
					422
				);
			}
		...
		protected function map_payment_button( string $default_button ): string {
			...
			<div class="learndash_checkout_button">
				<form
					...
					data-post_id="<?php echo esc_attr( (string) $this->post->ID ); ?>"
				>
		...
		protected function find_or_create_customer_id(): string {
			...
			try {
				$customer = $this->api->customer->create( // @phpstan-ignore-line -- magic method.
					array(
				...
				while ( is_null( $customer ) ) {
					$customers = $this->api->customer->all( // @phpstan-ignore-line -- magic method.
						array(
							'count' => $count,
							'skip'  => $skip,
						)
		...
		protected function create_order_id( int $amount ): string {
			$order = $this->api->order->create( // @phpstan-ignore-line -- magic method.
				array(
					'amount'          => $amount,
		...
		protected function create_subscription_id( int $price ): string {
			...
			return $this->api->subscription->create( $subscription_options )->id; // @phpstan-ignore-line -- magic method.
		}
		...
		protected function create_plan( array $plan_options, string $plan_options_hash, array $existing_plans ): string {
			$plan_id = $this->api->plan->create( $plan_options )->id; // @phpstan-ignore-line -- magic method.
