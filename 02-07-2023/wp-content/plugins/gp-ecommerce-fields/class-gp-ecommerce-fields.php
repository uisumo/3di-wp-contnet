...
class GP_Ecommerce_Fields extends GP_Plugin {
	...
	public function localize_frontend_script() {
		wp_localize_script( 'gp-ecommerce-fields', 'GPECF', array(
			...
			'ppcpRequiresDiscountFilter' => defined( 'GF_PPCP_VERSION' )
											&& (
												version_compare( GF_PPCP_VERSION, '2.2.1' ) === - 1
											) ? 'true' : 'false',
		) );
	}
	...
	public function reprocess_calculations( $order, $form, $entry ) {
		...
		foreach ( $form['fields'] as $field ) {
			...
			// origType is for gw-edit-product-and-payment-details.php snippet
			$is_product = $field['type'] === 'product' || rgar( $field, 'origType' ) === 'product';
