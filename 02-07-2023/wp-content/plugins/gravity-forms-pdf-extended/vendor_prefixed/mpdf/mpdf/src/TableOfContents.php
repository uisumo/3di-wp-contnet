...
class TableOfContents
{
    ...
    public function __construct(\GFPDF_Vendor\Mpdf\Mpdf $mpdf, \GFPDF_Vendor\Mpdf\SizeConverter $sizeConverter)
    {
        $this->mpdf = $mpdf;
    ...
    public function insertTOC()
    {
        ...
        if (!$this->tocTocPaintBegun) {
            $copier = new \GFPDF_Vendor\DeepCopy\DeepCopy(\true);
        ...
        for ($toci = 0; $toci < $notocs; $toci++) {
            if ($toci == 0 && $this->TOCmark) {
                ...
                $TOCuseLinking = $this->m_TOC[$toc_id]['TOCuseLinking'];
                $TOCusePaging = $this->m_TOC[$toc_id]['TOCusePaging'];
                $toc_bookmarkText = \GFPDF_Vendor\Mpdf\Utils\Arrays::get($this->m_TOC[$toc_id], 'TOCbookmarkText', null);
                // *BOOKMARKS*
