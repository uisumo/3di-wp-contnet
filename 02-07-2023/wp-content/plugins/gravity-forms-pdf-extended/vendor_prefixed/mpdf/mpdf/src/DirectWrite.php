...
class DirectWrite
{
    ...
    public function __construct(\GFPDF_Vendor\Mpdf\Mpdf $mpdf, \GFPDF_Vendor\Mpdf\Otl $otl, \GFPDF_Vendor\Mpdf\SizeConverter $sizeConverter, \GFPDF_Vendor\Mpdf\Color\ColorConverter $colorConverter)
    {
        $this->mpdf = $mpdf;
        $this->otl = $otl;
                ...
                if ($l > $wmax) {
                    ...
                    if ($sep == -1) {
                        ...
                        if ($align === 'J') {
                            ...
                            list($charspacing, $ws) = $this->mpdf->GetJspacing($nb_carac, $nb_spaces, ($w - 2 - $len_ligne) * \GFPDF_Vendor\Mpdf\Mpdf::SCALE, $inclCursive);
                ...
                if ($l > $wmax) {
                    ...
                    if ($sep == -1) {
                        ...
                        if ($align === 'J') {
                            ...
                            list($charspacing, $ws) = $this->mpdf->GetJspacing($nb_carac, $nb_spaces, ($w - 2 - $len_ligne) * \GFPDF_Vendor\Mpdf\Mpdf::SCALE, $false);
                            $this->mpdf->SetSpacing($charspacing, $ws);
                            //////////////////////////////////////////
                        }
                        $this->mpdf->Cell($w, $h, $tmp, 0, 2, $align, $fill, $link);
    ...
    function CircularText($x, $y, $r, $text, $align = 'top', $fontfamily = '', $fontsizePt = 0, $fontstyle = '', $kerning = 120, $fontwidth = 100, $divider = '')
    {
        ...
        $fontwidth /= 100;
        if ($kerning == 0) {
            throw new \GFPDF_Vendor\Mpdf\MpdfException('Please use values unequal to zero for kerning (CircularText)');
        }
        if ($fontwidth == 0) {
            throw new \GFPDF_Vendor\Mpdf\MpdfException('Please use values unequal to zero for font width (CircularText)');
        }
    ...
    function Shaded_box($text, $font = '', $fontstyle = 'B', $szfont = '', $width = '70%', $style = 'DF', $radius = 2.5, $fill = '#FFFFFF', $color = '#000000', $pad = 2)
    {
        ...
        if ($this->mpdf->useKerning) {
            if ($this->mpdf->CurrentFont['haskernGPOS']) {
                ...
                $textvar |= \GFPDF_Vendor\Mpdf\Css\TextVars::FC_KERNING;
            }
        }
