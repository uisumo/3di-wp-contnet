...
namespace GFPDF_Vendor\Mpdf\Conversion;
            ...
            for ($j = 0; $j < $n; $j++) {
                if (\is_array($additive_glyphs[$t])) {
                    foreach ($additive_glyphs[$t] as $ag) {
                        if ($reverse) {
                            $s = \GFPDF_Vendor\Mpdf\Utils\UtfString::code2utf($ag) . $s;
                        } else {
                            $s .= \GFPDF_Vendor\Mpdf\Utils\UtfString::code2utf($ag);
                        }
                    }
                } else {
                    if ($reverse) {
                        $s = \GFPDF_Vendor\Mpdf\Utils\UtfString::code2utf($additive_glyphs[$t]) . $s;
                    } else {
                        $s .= \GFPDF_Vendor\Mpdf\Utils\UtfString::code2utf($additive_glyphs[$t]);
                    }
                }
                $i -= $ct * $n;
            }
