...
use GFPDF_Vendor\QueryPath\CSS\SimpleSelector;
...
class DOMTraverser implements \GFPDF_Vendor\QueryPath\CSS\Traverser
{
     ...
     */
    public function find($selector) : \GFPDF_Vendor\QueryPath\CSS\DOMTraverser
    {
        // Setup
    ...
    public function combine(\DOMElement $node, $selectors, $index)
    {
        ...
        switch ($selector->combinator) {
            ...
            case \GFPDF_Vendor\QueryPath\CSS\SimpleSelector::DIRECT_DESCENDANT:
                return $this->combineDirectDescendant($node, $selectors, $index);
            case \GFPDF_Vendor\QueryPath\CSS\SimpleSelector::ANY_DESCENDANT:
                return $this->combineAnyDescendant($node, $selectors, $index);
            case \GFPDF_Vendor\QueryPath\CSS\SimpleSelector::ANOTHER_SELECTOR:
     ...
     */
    protected function initialMatch(\GFPDF_Vendor\QueryPath\CSS\SimpleSelector $selector, \SplObjectStorage $matches) : \SplObjectStorage
    {
     ...
     * @return SplObjectStorage
     */
    protected function initialMatchOnID(\GFPDF_Vendor\QueryPath\CSS\SimpleSelector $selector, \SplObjectStorage $matches) : \SplObjectStorage
    {
        $id = $selector->id;
        $found = $this->newMatches();
     ...
     * @return SplObjectStorage
     */
    protected function initialMatchOnClasses(\GFPDF_Vendor\QueryPath\CSS\SimpleSelector $selector, \SplObjectStorage $matches) : \SplObjectStorage
    {
        $found = $this->newMatches();
        // Issue #145: DOMXPath will through an exception if the DOM is
        // not set.
     ...
     * @return SplObjectStorage
     */
    protected function initialMatchOnElement(\GFPDF_Vendor\QueryPath\CSS\SimpleSelector $selector, \SplObjectStorage $matches) : \SplObjectStorage
    {
        $element = $selector->element;
        if (null === $element) {
            $element = '*';
     ...
     * @return SplObjectStorage
     */
    protected function initialMatchOnElementNS(\GFPDF_Vendor\QueryPath\CSS\SimpleSelector $selector, \SplObjectStorage $matches) : \SplObjectStorage
    {
        $ns = $selector->ns;
    ...
    protected function matchPseudoElements(\DOMElement $node, $pseudoElements) : bool
    {
        ...
        foreach ($pseudoElements as $pse) {
            switch ($pse) {
                ...
                case 'selection':
                    throw new \GFPDF_Vendor\QueryPath\CSS\NotImplementedException("::{$pse} is not implemented.");
            }
        }
        return \false;
    }
