 ...
 */
namespace GFPDF_Vendor\Masterminds\HTML5\Serializer;

use GFPDF_Vendor\Masterminds\HTML5\Elements;
/**
 * Generate the output html5 based on element rules.
 */
class OutputRules implements \GFPDF_Vendor\Masterminds\HTML5\Serializer\RulesInterface
{
    /**
    ...
    public function setTraverser(\GFPDF_Vendor\Masterminds\HTML5\Serializer\Traverser $traverser)
    {
    ...
    public function element($ele)
    {
        ...
        if ('svg' == $name) {
            ...
            $name = \GFPDF_Vendor\Masterminds\HTML5\Elements::normalizeSvgElement($name);
        } elseif ('math' == $name) {
            $this->outputMode = static::IM_IN_MATHML;
        }
        $this->openTag($ele);
        if (\GFPDF_Vendor\Masterminds\HTML5\Elements::isA($name, \GFPDF_Vendor\Masterminds\HTML5\Elements::TEXT_RAW)) {
    ...
    {
        if (isset($ele->parentNode) && isset($ele->parentNode->tagName) && \GFPDF_Vendor\Masterminds\HTML5\Elements::isA($ele->parentNode->localName, \GFPDF_Vendor\Masterminds\HTML5\Elements::TEXT_RAW)) {
            $this->wr($ele->data);
            return;
        }
    ...
    protected function attrs($ele)
    {
        ...
        for ($i = 0; $i < $len; ++$i) {
            ...
            if ($this->outputMode == static::IM_IN_SVG) {
                $name = \GFPDF_Vendor\Masterminds\HTML5\Elements::normalizeSvgAttribute($name);
            } elseif ($this->outputMode == static::IM_IN_MATHML) {
                $name = \GFPDF_Vendor\Masterminds\HTML5\Elements::normalizeMathMlAttribute($name);
            }
            $this->wr(' ')->wr($name);
    ...
    protected function enc($text, $attribute = \false)
    {
        ...
        if ($this->hasHTML5) {
            ...
            return \strtr($text, \GFPDF_Vendor\Masterminds\HTML5\Serializer\HTML5Entities::$map);
        }
    }
    /**
     * Escape test.
