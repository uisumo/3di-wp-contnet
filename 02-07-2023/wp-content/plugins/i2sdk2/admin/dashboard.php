...
function i2sdk_admin_menu() {
	...
	if ( $_SERVER['REQUEST_METHOD'] == 'POST' ) {
		...
		if ( ! empty( $_POST['save'] ) ) {
			...
			elseif ( $_POST['save'] == 'Save API Configuration' ) {
				...
				// TODO:  Add code to clean post key input of undesirable characters, possibly use built in WP function used to sanitize slugs
				...
				if ( isset( $_POST['i2sdk_email_notification'] ) ) {
					$i2sdk->setConfigurationOption( 'email_notification', ( in_array( (int) $_POST['i2sdk_email_notification'], array( 0, 1 ) )  ? (int) $_POST['i2sdk_email_notification'] : 0 ) );	
				}
	...
	// Determine What to Display On Screen
	...
	?>
	<div class="wrap">
	<h1>i<sup>2</sup>SDK <?php echo __( 'Enhanced Infusionsoft / WordPress Integration for Memberium' ); ?></h1>
	<div style="width:800px;">
		<form method="POST" action="" autocomplete="off">
			...
			<table class="widefat" style="white-space:nowrap;">
				...
				if ( $show_api_settings ) {
					...
					<tr>
						...
						<td>https://<input id=lastpass-search-1 autocomplete=off type=text maxlength=32 size=20 name=i2sdk_app_name value="<?php echo $i2sdk->getConfigurationOption( 'app_name' ) ?>" style="text-align:right;" />.infusionsoft.com/</td>
					</tr>
					<tr>
						<td><label for=""><?php _e( 'Infusionsoft Encrypted key' ); ?>:</label></td>
						<td><input id="lastpass-search-2" maxlength="255" autocomplete="off" name="i2sdk_api_key" size="40" type="password" value="<?php echo $i2sdk->getConfigurationOption( 'api_key' ) ?>" ></td>
					</tr>
					<?php
				}
				?>
			...
			?>
			<h2>Infusionsoft <?php _e( 'Install Plugins' ); ?></h2>
			<table class="widefat" style="white-space:nowrap;">
				<tr>
			...
			<table class="widefat" style="white-space:nowrap;">
				...
				if ( $i2sdk->getConfigurationOption( 'tracking_code' ) > '' ) {
					?>
					<tr>
						<td><label for=""><?php _e( 'Infusionsoft Web Analytics' ); ?>:</label></td>
						<td>
	...
	<?php
	// echo '<pre>' . print_r( $i2sdk, true ) . '</pre>';
	echo '</div>';
