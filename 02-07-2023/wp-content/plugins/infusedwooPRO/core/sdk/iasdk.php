...
class iaSDK
{
    ...
    public function cfgCon($name, $key = "", $dbOn = "on")
    {
        ...
        $this->client = new IWLib\PhpXmlRpc\Client("https://$appname.infusionsoft.com/api/xmlrpc");
        ...
        $this->encKey = $this->php_xmlrpc_encode($this->key);
    ...
    public function vendorCon($name, $user, $pass, $key = "", $dbOn = "on")
    {
        ...
        if ($key != "" && $key != "on" && $key != "off" && $key != 'kill' && $key != 'throw') {
            $this->client = new IWLib\PhpXmlRpc\Client("https://$name.infusionsoft.com/api/xmlrpc");
            $this->key = $key;
        } else {
            include('conn.cfg.php');
            $appLines = $connInfo;
            ...
            if (!empty($details[$name])) {
                if ($details[$name][2] == "i") {
                    $this->client = new IWLib\PhpXmlRpc\Client("https://" . $details[$name][1] .
        ...
        $carray = array(
            ...
            $this->php_xmlrpc_encode($user),
            $this->php_xmlrpc_encode(md5($pass)));
    ...
    {
        /* Set up the call */
        $call = new IWLib\PhpXmlRpc\Request($service, $callArray);
    ...
    public function affClawbacks($affId, $startDate, $endDate)
    {
        $carray = array(
            ...
            $this->php_xmlrpc_encode($startDate, array('auto_dates')),
            $this->php_xmlrpc_encode($endDate, array('auto_dates')));
    ...
    {
        $carray = array(
            $this->php_xmlrpc_encode($affList));
    ...
    public function addCon($cMap, $optReason = "")
    {
        ...
        $carray = array(
            $this->php_xmlrpc_encode($cMap, array('auto_dates')));
    ...
    {
        $carray = array(
            $this->php_xmlrpc_encode($cid),
            $this->php_xmlrpc_encode($dcid));
    ...
    public function loadCon($cid, $rFields)
    {
        ...
        $carray = array(
            $this->php_xmlrpc_encode((int)$cid),
            $this->php_xmlrpc_encode($rFields));
    ...
    public function resumeCampaignForContact($cid, $sequenceId)
    {
        $carray = array(
            $this->php_xmlrpc_encode((int)$cid),
            $this->php_xmlrpc_encode((int)$sequenceId));
    ...
    public function requestCcSubmissionToken($contactId, $successUrl, $failureUrl)
    {
        $carray = array(
            $this->php_xmlrpc_encode((int)$contactId),
            $this->php_xmlrpc_encode((string)$successUrl),
            $this->php_xmlrpc_encode((string)$failureUrl));
    ...
    public function dsAdd($tName, $iMap)
    {
        $carray = array(
            ...
            $this->php_xmlrpc_encode($iMap, array('auto_dates')));
    ...
    public function dsAddWithImage($tName, $iMap)
    {
        $carray = array(
            $this->php_xmlrpc_encode($tName),
            $this->php_xmlrpc_encode($iMap, array('auto_dates', 'auto_base64')));
    ...
    public function dsQuery($tName, $limit, $page, $query, $rFields)
    {
        $carray = array(
            ...
            $this->php_xmlrpc_encode((int)$limit),
            $this->php_xmlrpc_encode((int)$page),
            $this->php_xmlrpc_encode($query, array('auto_dates')),
            $this->php_xmlrpc_encode($rFields));
    ...
    public function addCustomField($context, $displayName, $dataType, $headerID)
    {
        $carray = array(
            ...
            $this->php_xmlrpc_encode((int)$headerID));
    ...
    {
        $carray = array(
            $this->php_xmlrpc_encode((string)$name),
            $this->php_xmlrpc_encode((string)$description),
    ...
    public function addOrderTotalDiscount($name, $description, $applyDiscountToCommission, $percentOrAmt, $amt, $payType)
    {
        $carray = array(
            ...
            $this->php_xmlrpc_encode($amt),
            $this->php_xmlrpc_encode($payType));
                                ...
                                $header, $strRecvdDate, $strSentDate, $emailSentType = 1)
    {
        $carray = array(
            ...
            $this->php_xmlrpc_encode($emailSentType));
                                        ...
                                        $txtBody)
    {
        $carray = array(
            ...
            $this->php_xmlrpc_encode($htmlBody),
            $this->php_xmlrpc_encode($contentType),
            $this->php_xmlrpc_encode($mergeContext = 'Contact'));
    ...
    {
        $carray = array(
            $this->php_xmlrpc_encode((string)$integration),
            $this->php_xmlrpc_encode((string)$callName),
            $this->php_xmlrpc_encode((int)$contactId));
    ...
    public function manualPmt($invId, $amt, $payDate, $payType, $payDesc, $bypassComm)
    {
        $carray = array(
            ...
            $this->php_xmlrpc_encode($amt),
            $this->php_xmlrpc_encode($payDate, array('auto_dates')),
            $this->php_xmlrpc_encode($payType),
            $this->php_xmlrpc_encode($payDesc),
            $this->php_xmlrpc_encode($bypassComm));
    ...
    public function commOverride($invId, $affId, $prodId, $percentage, $amt, $payType, $desc, $date)
    {
        $carray = array(
            ...
            $this->php_xmlrpc_encode($percentage),
            $this->php_xmlrpc_encode($amt),
            $this->php_xmlrpc_encode($payType),
            $this->php_xmlrpc_encode($desc),
            $this->php_xmlrpc_encode($date, array('auto_dates')));
    ...
    public function addOrderItem($ordId, $prodId, $type, $price, $qty, $desc, $notes)
    {
        $carray = array(
            ...
            $this->php_xmlrpc_encode($price),
            $this->php_xmlrpc_encode($qty),
            $this->php_xmlrpc_encode($desc),
    ...
    public function payPlan($ordId, $aCharge, $ccId, $merchId, $retry, $retryAmt, $initialPmt, $initialPmtDate, $planStartDate, $numPmts, $pmtDays)
    {
        $carray = array(
            $this->php_xmlrpc_encode((int)$ordId),
            $this->php_xmlrpc_encode($aCharge),
            ...
            $this->php_xmlrpc_encode($initialPmt),
            $this->php_xmlrpc_encode($initialPmtDate, array('auto_dates')),
            $this->php_xmlrpc_encode($planStartDate, array('auto_dates')),
    ...
    public function addRecurring($cid, $allowDup, $progId, $merchId, $ccId, $affId, $daysToCharge)
    {
        $carray = array(
            $this->php_xmlrpc_encode((int)$cid),
            $this->php_xmlrpc_encode($allowDup),
            $this->php_xmlrpc_encode((int)$progId),
            $this->php_xmlrpc_encode((int)$merchId),
            $this->php_xmlrpc_encode((int)$ccId),
            $this->php_xmlrpc_encode((int)$affId),
            $this->php_xmlrpc_encode($daysToCharge));
    ...
    public function addRecurringAdv($cid, $allowDup, $progId, $qty, $price, $allowTax, $merchId, $ccId, $affId, $daysToCharge)
    {
        $carray = array(
            ...
            $this->php_xmlrpc_encode($qty),
            $this->php_xmlrpc_encode($price),
            $this->php_xmlrpc_encode($allowTax),
            $this->php_xmlrpc_encode($merchId),
            $this->php_xmlrpc_encode((int)$ccId),
            $this->php_xmlrpc_encode((int)$affId),
            $this->php_xmlrpc_encode($daysToCharge));
        return $this->methodCaller("InvoiceService.addRecurringOrder", $carray);
    ...
    public function blankOrder($conId, $desc, $oDate, $leadAff, $saleAff)
    {
        $carray = array(
            ...
            $this->php_xmlrpc_encode($desc),
            $this->php_xmlrpc_encode($oDate, array('auto_dates')),
    ...
    public function updateSubscriptionNextBillDate($subscriptionId, $nextBillDate)
    {
        $carray = array(
            ...
            $this->php_xmlrpc_encode($nextBillDate, array('auto_dates')));
    ...
    public function placeOrder($contactId, $creditCardId, $payPlanId, $productIds, $subscriptionIds, $processSpecials, $promoCodes, $leadAff = 0, $saleAff = 0)
    {
        $carray = array(
            ...
            $this->php_xmlrpc_encode($processSpecials),
    ...
    {
        $carray = array(
            $this->php_xmlrpc_encode($quickSearchType),
            $this->php_xmlrpc_encode((int)$userId),
    ...
    private function php_xmlrpc_encode($phpVal, $options=array()) 
    {
        $encoder = new IWLib\PhpXmlRpc\Encoder();
