...
var window_width=Math.ceil(jQuery(window).width() * 1) - 22;
...
wp.data.dispatch('core/notices').createNotice('notice',
text,
{
isDismissible: true,
actions: actions,
...
var src=jQuery(element).data('data-src');
src +=(src.search(/[?]/) < 0)? "?":"&";
...
if(typeof el=="number"||jQuery(el).attr("class")=="grassblade_aspect_ratio"){
if(typeof el=="number")
var aspect=el;
else
...
if(jQuery(el).attr("id")=="aspect_slider_value"){
var aspect=jQuery(el).val() * 1;
jQuery("#aspect_slider").val(aspect);
jQuery("#aspect_slider_span").css('background', 'yellow');
setTimeout(function(){
...
if(iframe_content.contentDocument!=null){
...
var myHilitor=new Hilitor("grassblade_settings_form");
myHilitor.apply(search);
}
...
if(file_version > 0){
if(jQuery(context).parent().hasClass("file_version_selected"))
